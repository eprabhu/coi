...
END IF;
	...
	IF AV_APPROVAL_STATUS = 'W' THEN

				SET LI_DELEGATED = FN_WORKFLOW_HANDLE_DELEGATIONS(AV_WORKFLOW_ID, AV_MAP_NUMBER, AV_APPROVAL_STOP_NUMBER, AV_APPROVAL_STATUS, (SELECT PERSON_ID FROM PERSON WHERE USER_NAME = AV_UPDATE_USER),AV_UPDATE_USER);
	...
	-- -----------------------STARTS MODIFICATION ON 07-MAY-2020-------------------------
		...
		WHERE T1.MODULE_ITEM_KEY = (SELECT AWARD_ID FROM AWARD 
		...
		IF LI_COUNT > 1 THEN 
			...
			WHERE T1.MODULE_ITEM_KEY = (SELECT AWARD_ID FROM AWARD 
									 WHERE AWARD_NUMBER = LS_AWARD_NUMBER
									 AND AWARD_SEQUENCE_STATUS = 'ACTIVE')
			AND T1.ORIGINATING_MODULE_ITEM_KEY = LS_MODULE_ITEM_ID
			AND T1.MODULE_CODE = 1;
		ELSE 
			SELECT T2.DESCRIPTION INTO LS_SERVICE_REQUEST_TYPE FROM SR_HEADER T1
			INNER JOIN SR_TYPE T2 ON T1.TYPE_CODE = T2.TYPE_CODE
