...
END IF;
					SET LS_DYN_SQL = CONCAT('WITH ACCESS_TMP 
							...
							');
		 set LS_DYN_SQL = concat(LS_DYN_SQL, 'SELECT UNIT_NUMBER,UNIT_NAME FROM UNIT WHERE (UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM ACCESS_TMP)
                            AND ACTIVE_FLAG = ''Y'') AND (LOWER(UNIT_NUMBER) like LOWER(''%',AV_SEARCH,'%'') OR LOWER(UNIT_NAME) like LOWER(''%',AV_SEARCH,'%'')) ORDER BY UNIT_NAME ASC');
