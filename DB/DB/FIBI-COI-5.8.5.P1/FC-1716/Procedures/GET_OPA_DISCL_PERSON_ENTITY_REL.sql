...
CREATE PROCEDURE `GET_OPA_DISCL_PERSON_ENTITY_REL`(
    ...
    IN AV_FILTER_TYPE               VARCHAR(20),
    IN AV_TAB_TYPE                  VARCHAR(20),
...
BEGIN
 ...
 DECLARE LS_PERSON_ID VARCHAR(40);
    ...
    SELECT REVIEW_STATUS_CODE, PERSON_ID
    INTO LI_REVIEW_STATUS_CODE, LS_PERSON_ID
    FROM OPA_DISCLOSURE
    ...
    FROM 
      ...
      AND (
        LI_REVIEW_STATUS_CODE IN ('1','5', '6')
...
A.IS_SPRING_SABATICAL,
EOT.DESCRIPTION AS ENTITY_OWNERSHIP_TYPE
  FROM PERSON_ENTITIES A
  LEFT JOIN PERSON_ENTITY_DELETE_ELIGIBILITY B ON A.PERSON_ENTITY_ID = B.PERSON_ENTITY_ID
INNER JOIN PERSON_ENTITY PE ON A.PERSON_ENTITY_ID = PE.PERSON_ENTITY_ID
LEFT JOIN ENTITY E ON A.ENTITY_ID = E.ENTITY_ID
LEFT JOIN PER_ENT_DISCL_TYPE_SELECTION PETS ON PETS.PERSON_ENTITY_ID = PE.PERSON_ENTITY_ID
LEFT JOIN (SELECT GROUP_CONCAT(DISTINCT t3.DESCRIPTION ORDER BY t3.DESCRIPTION SEPARATOR ', ') AS RELATIONSHIPS, t1.PERSON_ENTITY_ID
    FROM PERSON_ENTITY t1
    ...
    WHERE t1.PERSON_ID = LS_PERSON_ID
    GROUP BY t1.PERSON_ENTITY_ID
) REL ON REL.PERSON_ENTITY_ID = PE.PERSON_ENTITY_ID
LEFT JOIN COUNTRY C ON E.COUNTRY_CODE = C.COUNTRY_CODE
LEFT JOIN ENTITY_OWNERSHIP_TYPE EOT ON EOT.OWNERSHIP_TYPE_CODE = E.ENTITY_OWNERSHIP_TYPE_CODE
WHERE
    CASE
        WHEN AV_TAB_TYPE <> 'OPA' THEN
            CASE
                WHEN LI_REVIEW_STATUS_CODE IN ('2', '3', '4', '7', '8', '9') THEN TRUE
                WHEN AV_FILTER_TYPE = 'ALL' THEN TRUE
                WHEN AV_FILTER_TYPE = 'COMPLETE' THEN
                    PE.VERSION_STATUS = 'ACTIVE' AND PE.IS_FORM_COMPLETED = 'Y'
                WHEN AV_FILTER_TYPE = 'INCOMPLETE' THEN
                    PE.VERSION_STATUS = 'ACTIVE' AND PE.IS_FORM_COMPLETED = 'N'
                WHEN AV_FILTER_TYPE = 'INACTIVE' THEN
                    PE.VERSION_STATUS = 'INACTIVE'
                WHEN AV_FILTER_TYPE = 'ACTIVE' THEN
                    PE.VERSION_STATUS = 'ACTIVE'
                WHEN AV_FILTER_TYPE = 'NON_OPA_COMPLETED' THEN
                    PE.VERSION_STATUS = 'ACTIVE'
                    AND PE.IS_FORM_COMPLETED = 'Y'
                    AND (
                        PETS.DISCLOSURE_TYPE_CODE IS NULL OR
                        NOT EXISTS (
                            SELECT 1
                            FROM PER_ENT_DISCL_TYPE_SELECTION t1
                            WHERE t1.PERSON_ENTITY_ID = PE.PERSON_ENTITY_ID
                              AND t1.DISCLOSURE_TYPE_CODE = 2
                        )
                    )
                ELSE TRUE
            END
        ELSE
            CASE
                WHEN LI_REVIEW_STATUS_CODE IN ('2', '3', '4', '7', '8', '9') THEN TRUE
                WHEN AV_FILTER_TYPE = 'ALL' THEN
                    PETS.DISCLOSURE_TYPE_CODE = 2
                WHEN AV_FILTER_TYPE = 'COMPLETE' THEN
                    PE.VERSION_STATUS = 'ACTIVE' AND PE.IS_FORM_COMPLETED = 'Y' AND PETS.DISCLOSURE_TYPE_CODE = 2
                WHEN AV_FILTER_TYPE = 'INCOMPLETE' THEN
                    PE.VERSION_STATUS = 'ACTIVE' AND PE.IS_FORM_COMPLETED = 'N' AND PETS.DISCLOSURE_TYPE_CODE = 2
                WHEN AV_FILTER_TYPE = 'INACTIVE' THEN
                    PE.VERSION_STATUS = 'INACTIVE' AND PETS.DISCLOSURE_TYPE_CODE = 2
                WHEN AV_FILTER_TYPE = 'ACTIVE' THEN
                    PE.VERSION_STATUS = 'ACTIVE' AND PETS.DISCLOSURE_TYPE_CODE = 2
                ELSE TRUE
            END
    END
    AND (
