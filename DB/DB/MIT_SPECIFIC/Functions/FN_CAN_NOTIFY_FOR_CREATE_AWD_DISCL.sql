...
BEGIN
    ...
    SET MODULE_PREFIX = SUBSTRING_INDEX(AV_MODULE_ITEM_KEY, '-', 1);
		...
		WHERE T2.KEY_PERSON_ID = AV_PERSON_ID AND T2.STATUS = 'A' AND T2.ATTRIBUTE_1_VALUE = 'Y' AND T1.PROJECT_NUMBER = AV_MODULE_ITEM_KEY  AND T1.PROJECT_STATUS_CODE IN (6);
		...
		SELECT COUNT(T1.PROJECT_NUMBER) INTO LI_COUNT 
		FROM COI_INT_STAGE_AWARD T1
		WHERE T1.PROJECT_NUMBER = AV_MODULE_ITEM_KEY AND T1.PROJECT_STATUS_CODE IN (6);
		...
		FROM COI_INT_STAGE_AWARD T1
		INNER JOIN COI_INT_STAGE_AWARD_PERSON T2 ON T2.PROJECT_NUMBER = T1.PROJECT_NUMBER
		WHERE T2.KEY_PERSON_ID = AV_PERSON_ID AND T2.STATUS = 'A' AND T1.PROJECT_NUMBER = AV_MODULE_ITEM_KEY  AND T1.PROJECT_STATUS_CODE IN (6);
		...
		ELSEIF LS_REQUIRED_FLAG = 'REQUIRED' AND (LS_DISCLOSURE_VALIDATION_FLAG IS NULL OR LS_DISCLOSURE_VALIDATION_FLAG = 'HIERARCHY') THEN
				...
				WHERE TO_PERSON_ID = AV_PERSON_ID AND MODULE_CODE = AV_MODULE_CODE
				  ...
				  AND OPENED_FLAG = 'N' AND SUBSTRING_INDEX(MODULE_ITEM_KEY, '-', 1) = MODULE_PREFIX;
					...
					IF LI_COUNT = 0 THEN  
                     SELECT COUNT(T1.LEGACY_DISCLOSURE_ID) INTO LI_COUNT
					 FROM LEGACY_COI_DISCLOSURE T1
					 INNER JOIN LEGACY_COI_DISC_DETAILS T2 ON T2.LEGACY_DISCLOSURE_ID = T1.LEGACY_DISCLOSURE_ID
                     WHERE T1.PERSON_ID = AV_PERSON_ID AND T2.MODULE_CODE = 1
                     AND T1.DISCLOSURE_DISPOSITION_CODE = 1
                     AND SUBSTRING_INDEX(T2.MODULE_ITEM_KEY, '-', 1) = MODULE_PREFIX;
