...
BEGIN
    ...
    SET LS_JOIN_CONDITION = CONCAT(
        ' INNER JOIN COI_PROJECT_TYPE T2 ON T2.COI_PROJECT_TYPE_CODE = T1.COI_PROJECT_TYPE_CODE
          LEFT JOIN (SELECT DISTINCT AW2.PROJECT_NUMBER 
                     ...
                     WHERE AW2.PROJECT_STATUS_CODE = 6 
                       AND AW3.KEY_PERSON_ID = (CONVERT(''', AV_PERSON_ID, ''' USING utf8mb3) COLLATE utf8mb3_general_ci)
    ...
    SET LS_FILTER_CONDITION = CONCAT(
        ' WHERE T1.KEY_PERSON_ID = (CONVERT(''', AV_PERSON_ID, ''' USING utf8mb3) COLLATE utf8mb3_general_ci) 
          ...
          OR (T1.DISCLOSURE_REQUIRED_FLAG = ''REQUIRED'' 
              ...
              AND T1.AWARD_STATUS_CODE = 6)) '
    );

    
    IF AV_SEARCHWORD IS NOT NULL THEN
        SET AV_SEARCHWORD = CONCAT('%', AV_SEARCHWORD, '%');
        SET SEARCH_CONDITION = CONCAT(
            ' AND (PROJECT_NUMBER LIKE (CONVERT(''', AV_SEARCHWORD, ''' USING utf8mb3) COLLATE utf8mb3_general_ci)
                   OR TITLE LIKE (CONVERT(''', AV_SEARCHWORD, ''' USING utf8mb3) COLLATE utf8mb3_general_ci)
                   OR LEAD_UNIT_NAME LIKE (CONVERT(''', AV_SEARCHWORD, ''' USING utf8mb3) COLLATE utf8mb3_general_ci))'
        );
    END IF;
    ...
    SET LS_DYN_SQL = CONCAT(
        ...
        ' FROM (WITH Awards AS (
             SELECT DISTINCT T1.EXTERNAL_SYSTEM_REF_ID AS PROJECT_ID,
                             ...
                             T1.UPDATE_TIMESTAMP, T1.DISCLOSURE_VALIDATION_FLAG, T1.DISCLOSURE_REQUIRED_FLAG,
                             T2.PROJECT_ICON, T1.DOCUMENT_NUMBER, T1.ACCOUNT_NUMBER,
                             ...
                             T1.NEW_DISCLOSURE_REQUIRED, T1.AWARD_STATUS_CODE
              FROM COI_PROJECT_AWARD_V T1 ', 
              LS_JOIN_CONDITION,'', LS_FILTER_CONDITION, ') 
           SELECT * FROM Awards 
           WHERE AWARD_STATUS_CODE NOT IN (2,5) 
             AND PROJECT_NUMBER NOT IN (
                     ...
                     WHERE MODULE_CODE = 1 
                       AND C1.DISPOSITION_STATUS_CODE <> 2 
                       AND PERSON_ID = (CONVERT(''', AV_PERSON_ID, ''' USING utf8mb3) COLLATE utf8mb3_general_ci)
                       AND (C3.NEW_DISCLOSURE_REQUIRED IS NULL OR C3.NEW_DISCLOSURE_REQUIRED = ''N'') 
                       AND C3.DISCLOSURE_REQUIRED_FLAG = ''REQUIRED''
             ...
             AND NOT EXISTS (
                     ...
                     WHERE DISPOSITION_STATUS_CODE = 1 
                       AND PERSON_ID = (CONVERT(''', AV_PERSON_ID, ''' USING utf8mb3) COLLATE utf8mb3_general_ci)
                       AND PROJECT_NUMBER = CD2.MODULE_ITEM_KEY
                  )
             AND ((DISCLOSURE_VALIDATION_FLAG = ''SELF'' AND NEW_DISCLOSURE_REQUIRED = ''Y'')
                  OR (DISCLOSURE_REQUIRED_FLAG = ''REQUIRED'' 
                      AND (DISCLOSURE_VALIDATION_FLAG IS NULL OR DISCLOSURE_VALIDATION_FLAG = ''HIERARCHY'') 
                      AND rn = 1 
                      AND NOT EXISTS (
                          SELECT 1 
                          FROM Awards AR 
                      ...
                      AND NOT EXISTS (
                          SELECT 1
                          FROM COI_DISCLOSURE CDH
                          INNER JOIN COI_DISCL_PROJECTS CDP ON CDP.DISCLOSURE_ID = CDH.DISCLOSURE_ID
                          INNER JOIN COI_PROJECT_AWARD_V CPA ON CPA.AWARD_NUMBER = CDP.MODULE_ITEM_KEY
                          WHERE CDP.MODULE_CODE = 1 
                            AND CDH.DISPOSITION_STATUS_CODE <> 2
                            AND CDH.PERSON_ID = (CONVERT(''', AV_PERSON_ID, ''' USING utf8mb3) COLLATE utf8mb3_general_ci)
                            AND CPA.DISCLOSURE_REQUIRED_FLAG = ''REQUIRED''
                            AND (CPA.DISCLOSURE_VALIDATION_FLAG IS NULL OR CPA.DISCLOSURE_VALIDATION_FLAG = ''HIERARCHY'')
                            AND SUBSTRING_INDEX(CPA.AWARD_NUMBER, ''-'', 1) = SUBSTRING_INDEX(Awards.PROJECT_NUMBER, ''-'', 1)
                      )
                  )
             ) ', SEARCH_CONDITION, 
        ') T ORDER BY T.UPDATE_TIMESTAMP DESC ', LS_OFFSET_CONDITION
    );
