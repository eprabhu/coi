DELIMITER //
CREATE PROCEDURE `REVISE_OPA_DISCLOSURE_DETAILS`(
AV_PERSON_ID                     	VARCHAR(40),
AV_LOGGEDIN_USER					VARCHAR(60),
AV_OPA_DISCLOSURE_NUMBER			VARCHAR(20)
)
BEGIN
...
SET LS_HOME_UNIT = FN_GET_OPA_DISCLOSURE_HOME_UNIT(AV_PERSON_ID);

IF LS_HOME_UNIT IS NULL OR LS_HOME_UNIT = '' THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No home unit found for this person. Contact the administrator for assistance.', MYSQL_ERRNO = 406;
END IF;


SELECT MAX(OPA_CYCLE_NUMBER) INTO LI_OPA_CYCLE_NUMBER FROM OPA_CYCLES;

SELECT MAX(VERSION_NUMBER) INTO LS_VERSION_NUMBER FROM OPA_DISCLOSURE WHERE OPA_DISCLOSURE_NUMBER = AV_OPA_DISCLOSURE_NUMBER;

INSERT INTO OPA_DISCLOSURE(OPA_DISCLOSURE_NUMBER, OPA_CYCLE_NUMBER, PERSON_ID, HOME_UNIT, CREATE_USER, CREATE_TIMESTAMP, UPDATED_BY, UPDATE_TIMESTAMP, REVIEW_STATUS_CODE, DISPOSITION_STATUS_CODE, VERSION_STATUS, VERSION_NUMBER)
VALUES 
(CONCAT(LS_PREFIX, AV_OPA_DISCLOSURE_NUMBER), LI_OPA_CYCLE_NUMBER, AV_PERSON_ID, LS_HOME_UNIT, AV_LOGGEDIN_USER, UTC_TIMESTAMP(), AV_PERSON_ID, UTC_TIMESTAMP(), LS_OPA_REVIEW_STATUS_CODE, LS_DISPOSITION_STATUS_CODE, LS_VERSION_STATUS, LS_VERSION_NUMBER + 1);

SELECT LAST_INSERT_ID() INTO LI_OPA_DISCLOSURE_ID;

INSERT INTO OPA_DISCL_PERSON_SNAP (OPA_DISCLOSURE_ID, PERSON_ID, JOB_TITLE, HR_ORG_UNIT_ID, HR_DEPARTMENT_NAME, ADMIN_EMPLOYEE_TYPE, ADMIN_ORG_UNIT_TITLE, ADMIN_POSITION_TITLE, EMPLOYMENT_PERCENT, IS_CONSULT_PRIV, IS_PAID_APPT, IS_SUMMER_SESSION_APPT, OPA_REQUIREMENT_REASON, SABBATICAL_BEGIN_DATE, SABBATICAL_END_DATE, CREATE_TIMESTAMP, UPDATE_TIMESTAMP, PERSONNEL_SUBAREA, PERSONNEL_SUBAREA_CODE)
SELECT LI_OPA_DISCLOSURE_ID, PERSON_ID, JOB_TITLE, HR_ORG_UNIT_ID, HR_DEPARTMENT_NAME, ADMIN_EMPLOYEE_TYPE, ADMIN_ORG_UNIT_TITLE, ADMIN_POSITION_TITLE, EMPLOYMENT_PERCENT, IS_CONSULT_PRIV, IS_PAID_APPT, IS_SUMMER_SESSION_APPT, REQUIREMENT_REASON, SABBATICAL_BEGIN_DATE, SABBATICAL_END_DATE, UTC_TIMESTAMP(), UTC_TIMESTAMP(), PERSONNEL_SUBAREA, PERSONNEL_SUBAREA_CODE
