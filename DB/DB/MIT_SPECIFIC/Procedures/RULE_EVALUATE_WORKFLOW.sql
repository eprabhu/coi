...
elseif AV_MODULE_CODE = 23 then
	 ...
	 FROM OPA_DISCLOSURE WHERE OPA_DISCLOSURE_ID = AV_MODULE_ITEM_KEY;

elseif AV_MODULE_CODE = 8 then

	 SELECT HOME_UNIT, FCOI_TYPE_CODE INTO LS_LEAD_UNIT_NUMBER, LS_FCOI_TYPE_CODE
...
IF LS_WORKFLOW_FLAG <> 0 THEN
		...
		ELSEIF AV_MODULE_CODE = 20 THEN	
				...
				SET LS_MSG_TYPE := '135';
		ELSEIF AV_MODULE_CODE = 23 THEN
                SET LS_MSG_TYPE := '8027';
        ELSEIF AV_MODULE_CODE = 8 THEN
            IF LS_FCOI_TYPE_CODE = 2 THEN
                SET LS_MSG_TYPE := '8037';
            ELSE
                SET LS_MSG_TYPE := '8036';
            END IF;
		END IF;
    ...
    ELSEIF AV_MODULE_CODE = 23 THEN  -- OPA
        SELECT CONCAT('OPA Disclosure of ', t2.FULL_NAME, ' submitted on ', DATE_FORMAT(UTC_TIMESTAMP(), '%m/%d/%Y')) into LS_USER_MESSAGE FROM OPA_DISCLOSURE t1
        INNER JOIN PERSON t2 ON t2.PERSON_ID = t1.PERSON_ID
        WHERE t1.OPA_DISCLOSURE_ID = AV_MODULE_ITEM_KEY;

    ELSEIF AV_MODULE_CODE = 8 THEN  -- COI Disclosure
            IF LS_FCOI_TYPE_CODE = 2 THEN
                SELECT CONCAT('Project disclosure for ', t3.DESCRIPTION, ' : ', COALESCE(t5.AWARD_NUMBER, t6.PROPOSAL_NUMBER), ' - ',
                COALESCE(t5.TITLE, t6.TITLE), ' of ', t2.FULL_NAME, ' submitted on ', DATE_FORMAT(UTC_TIMESTAMP(), '%m/%d/%Y')) into LS_USER_MESSAGE
                ...
                WHERE t1.DISCLOSURE_ID = AV_MODULE_ITEM_KEY;
            ELSE
                SELECT CONCAT('FCOI disclosure of ', t2.FULL_NAME, ' submitted on ', DATE_FORMAT(UTC_TIMESTAMP(), '%m/%d/%Y')) into LS_USER_MESSAGE
                FROM COI_DISCLOSURE t1
                INNER JOIN PERSON t2 ON t2.PERSON_ID = t1.PERSON_ID
                WHERE t1.DISCLOSURE_ID = AV_MODULE_ITEM_KEY;
            END IF;
