...
DECLARE CUR_CITI_DATA CURSOR FOR
	SELECT T1.CUSTOM_FIELD_2, (CONVERT(T1.GROUP USING latin1) COLLATE latin1_swedish_ci), T1.STAGE_NUMBER, STR_TO_DATE(T1.DATE_COMPLETED, '%m/%d/%Y %r') ,CASE WHEN T1.PASSING_SCORE < T1.EXAM_SCORE THEN 'P' ELSE 'F' END AS SCORE
                       ...
                       ELSEIF (LI_MODULE_CODE = 104) THEN
                        ...
                        IF (LI_COUNT_HUMAN = 0) and (LI_COUNT_GCP = 0) and (LI_COUNT_RCR = 0) and (LI_COUNT_COI = 0) and (LI_COUNT_UFI = 0) and (LI_COUNT_IACUC = 0 and LI_COUNT_EXPORT = 0 ) THEN
                                ...
                                IF (LI_MODULE_CODE = 7) THEN
									...
									VALUES (LS_PERSON_ID, LI_TRAINING_NO + 1,  LI_TRAINING_CODE, LD_DATE_COMPLETED, LD_DATE_COMPLETED, LD_FOLLOWUP_DATE, LI_SCORE, UTC_TIMESTAMP(), 'vineeth','Y');
