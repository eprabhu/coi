...
IF AV_TYPE ='A' THEN
    ...
    IF AV_UNIT IS NOT NULL AND AV_UNIT <> '' THEN
				IF FIND_IN_SET('UNIT', AV_FREE_TEXT_SEARCH_FIELDS) > 0 THEN
					...
					SET LS_FILTER_CONDITION = CONCAT(
						...
						''' OR T1.LEAD_UNIT LIKE ''', AV_UNIT, ''') '
					);
				ELSE
					SET LS_FILTER_CONDITION = CONCAT(
						LS_FILTER_CONDITION,
						' AND T1.LEAD_UNIT = ''', AV_UNIT, ''' '
					);
				END IF;
			END IF;
    ...
    IF AV_PERSON IS NOT NULL AND AV_PERSON <> '' THEN
            IF FIND_IN_SET('PERSON', AV_FREE_TEXT_SEARCH_FIELDS) > 0 THEN
                ...
                SET LS_FILTER_CONDITION = CONCAT(
                    LS_FILTER_CONDITION,
                    ' AND T1.PROPOSAL_NUMBER IN (',
                        'SELECT PROPOSAL_NUMBER ',
                        'FROM COI_INT_STAGE_DEV_PROPOSAL_PERSON ',
                        'WHERE KEY_PERSON_NAME LIKE ''', AV_PERSON, ''' ',
                        'OR KEY_PERSON_ID LIKE ''', AV_PERSON, '''',
                    ') '
                );
            ELSE
                SET LS_FILTER_CONDITION = CONCAT(
                    LS_FILTER_CONDITION,
                ' AND T1.PROPOSAL_NUMBER IN(SELECT PROPOSAL_NUMBER FROM COI_INT_STAGE_DEV_PROPOSAL_PERSON WHERE KEY_PERSON_ID = ''',AV_PERSON,''') '
                );
            END IF;
        END IF;
   ...
   IF AV_SPONSOR IS NOT NULL AND AV_SPONSOR <> '' THEN
            ...
            END IF;
                SET LS_FILTER_CONDITION = CONCAT(
                    LS_FILTER_CONDITION,
                    ' AND (T1.SPONSOR LIKE ''', AV_SPONSOR,
                    ''' OR T1.SPONSOR_CODE LIKE ''', AV_SPONSOR, ''') '
                );
            ELSE
                SET LS_FILTER_CONDITION = CONCAT(
                    LS_FILTER_CONDITION,
                        ' AND T1.SPONSOR_CODE = ''', AV_SPONSOR, ''' '
                );
            END IF;
        END IF;
        ...
        END IF;
            SET LS_FILTER_CONDITION = CONCAT(
                LS_FILTER_CONDITION,
                ' AND (T1.PRIME_SPONSOR LIKE ''', AV_PRIME_SPONSOR,
                ''' OR T1.PRIME_SPONSOR_CODE LIKE ''', AV_PRIME_SPONSOR, ''') '
            );
        ELSE
    ...
    IF AV_PI_PERSON IS NOT NULL AND AV_PI_PERSON <> '' THEN
                IF FIND_IN_SET('PRINCIPAL_INVESTIGATOR', AV_FREE_TEXT_SEARCH_FIELDS) > 0 THEN
                    ...
                    SET LS_FILTER_CONDITION = CONCAT(
                    LS_FILTER_CONDITION,
                    ' AND (CISDPP.KEY_PERSON_NAME LIKE ''', AV_PI_PERSON,
                    ''' OR CISDPP.KEY_PERSON_ID LIKE ''', AV_PI_PERSON, ''') ',
                    ' AND CISDPP.KEY_PERSON_ROLE_CODE = 3'
                );
                ELSE
                    SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' AND (CISDPP.KEY_PERSON_ID = ''',AV_PI_PERSON,''' AND CISDPP.KEY_PERSON_ROLE_CODE = 3 ) ');
                END IF;
            END IF;	      
...
T1.PROJECT_ID,
    ...
    END AS PERSON_SUBMISSION_STATUS ,
 	CASE WHEN T1.REVIEW_STATUS = ''Completed'' THEN ''Approved''
    ...
    WHEN T1.REVIEW_STATUS = ''DISCLOSURE_PENDING'' THEN ''In Progress''
    ...
    WHEN T1.PROJECT_REVIEW_STATUS = ''DISCLOSURE_COMPLETED'' THEN ''Approved''
		...
		WHEN ''',LI_TRAINING_CODE,''' IS NOT NULL AND ''',LI_TRAINING_CODE,''' <> '''' THEN
			CASE 
                ...
                CASE
                    WHEN EXISTS (
                        SELECT 1
                        FROM SPONSOR_DISCLOSURE_REQUIREMENTS SDR
                        WHERE SDR.SPONSOR_CODE IN (T1.SPONSOR_CODE, T1.PRIME_SPONSOR_CODE)
                          AND SDR.KEY_PERSON_DISCL_REQUIREMENT = ''ALL''
                    ) THEN ''Training Required''
                    WHEN EXISTS (
                        SELECT 1
                        FROM SPONSOR_DISCLOSURE_REQUIREMENTS SDR
                        WHERE SDR.SPONSOR_CODE IN (T1.SPONSOR_CODE, T1.PRIME_SPONSOR_CODE)
                          AND SDR.KEY_PERSON_DISCL_REQUIREMENT = ''PI-COI''
                          AND CISDPP.KEY_PERSON_ROLE_CODE IN (''1'',''3'')
                    ) THEN ''Training Not Required''
