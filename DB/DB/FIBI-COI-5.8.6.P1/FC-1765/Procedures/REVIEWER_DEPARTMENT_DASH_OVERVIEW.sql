...
*/
   ...
   -- SET LS_PERSON_FILTER_CONDITION = CONCAT(' EXISTS(SELECT 1 FROM PERSON p WHERE p.HOME_UNIT = U.UNIT_NUMBER) AND ');
    ...
    SET LS_DYN_CTE_SQL = CONCAT('
        WITH UNIT_ACCESS_TMP AS (SELECT t1.UNIT_NUMBER,t1.UNIT_NAME,t1.PARENT_UNIT_NUMBER, t1.DISPLAY_NAME, t1.IS_ACTIVE,1 AS LVL
            FROM UNIT t1 WHERE t1.PARENT_UNIT_NUMBER is null AND EXISTS(SELECT 1 FROM PERSON WHERE STATUS = ''A'' AND HOME_UNIT = t1.UNIT_NUMBER)
            UNION ALL
            SELECT t1.UNIT_NUMBER,t1.UNIT_NAME,t1.PARENT_UNIT_NUMBER, t1.DISPLAY_NAME, t1.IS_ACTIVE,FN_GET_UNIT_HIERARCHY_LEVEL(UNIT_NUMBER) AS LVL
            FROM UNIT t1
            WHERE t1.PARENT_UNIT_NUMBER is not null AND EXISTS(SELECT 1 FROM PERSON WHERE STATUS = ''A'' AND HOME_UNIT = t1.UNIT_NUMBER)
