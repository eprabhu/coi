...
BEGIN
    ...
    IF AV_TAB_TYPE IS NULL OR AV_TAB_TYPE <> 'DELINQUENT' THEN
        ...
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION, ' AND EXISTS (SELECT 1
            ...
            WHERE PR.PERSON_ID = ''', AV_LOGIN_PERSON_ID, '''
              AND (
                  (PR.DESCEND_FLAG = ''N'' AND R.RIGHT_NAME IN (''VIEW_OPA_ELIGIBILITY'', ''MAINTAIN_OPA_ELIGIBILITY'') AND PR.UNIT_NUMBER = T3.HOME_UNIT)
                  OR (
                      PR.DESCEND_FLAG = ''Y''
                      AND R.RIGHT_NAME IN (''VIEW_OPA_ELIGIBILITY'', ''MAINTAIN_OPA_ELIGIBILITY'')
                      AND EXISTS (
                          SELECT 1
                          FROM UNIT_WITH_CHILDREN UC
                          WHERE UC.UNIT_NUMBER = PR.UNIT_NUMBER
                            AND UC.CHILD_UNIT_NUMBER = T3.HOME_UNIT
                      )
                  )
              ))');
    ...
    ELSEIF AV_TAB_TYPE = 'DELINQUENT' THEN
        ...
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION, ' AND EXISTS  (SELECT 1
            ...
            WHERE PR.PERSON_ID = ''', AV_LOGIN_PERSON_ID, '''
              AND (
                  (PR.DESCEND_FLAG = ''N'' AND R.RIGHT_NAME IN (''MANAGE_OPA_DISCLOSURE'', ''VIEW_OPA_DISCLOSURE'') AND PR.UNIT_NUMBER = T3.HOME_UNIT)
                  OR (
                      PR.DESCEND_FLAG = ''Y''
    ...
    ELSEIF AV_INCLUDE_CHILD_UNITS = TRUE THEN
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' AND T2.UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM UNIT_WITH_CHILDREN WHERE UNIT_NUMBER =''',AV_HOME_UNIT,''') ');
    END IF;
    ...
    ELSE
        SET SELECTED_FIELD_LIST = 'T1.PERSON_DISCL_REQUIREMENT_ID,
                T1.PERSON_ID,
    ...
    SET LS_DYN_SQL = CONCAT(LS_DYN_CTE_SQL,' SELECT * FROM (SELECT ', SELECTED_FIELD_LIST,' FROM PERSON_DISCL_REQUIREMENT T1
