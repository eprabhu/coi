...
BEGIN
    ...
    DECLARE ATT_CUR CURSOR FOR
        SELECT
            ATT_ANSWER.MODULE_ITEM_CODE,
            ATT_ANSWER.MODULE_SUB_ITEM_CODE,
            cmp.FORM_BUILDER_SECT_COMP_ID AS MODULE_SUB_ITEM_KEY,
            ATT_ANSWER.FILE_ID,
            ATT_ANSWER.FILE_NAME,
            ATT_ANSWER.DESCRIPTION
        FROM FB_ATTACHMENTS  ATT_ANSWER
        INNER JOIN FORM_BUILDER_SECTION_COMPONENT COMPONENT
            ON ATT_ANSWER.MODULE_SUB_ITEM_KEY = COMPONENT.FORM_BUILDER_SECT_COMP_ID
        ...
        AND ATT_ANSWER.MODULE_ITEM_KEY = AV_OLD_MODULE_ITEM_KEY;
    ...
    READ_LOOP: LOOP
        ...
        CALL UPD_FB_ANS_TO_NEW_QUEST_VERSN(
			LS_NEW_QNR_ID, 
			LI_QUESTIONNAIRE_ANS_HEADER_ID, 
			AV_UPDATE_USER, 
			LS_NEW_FB_SECTION_COMPONENT_ID, 
			CASE 
				WHEN AV_OLD_MODULE_ITEM_KEY = AV_NEW_MODULE_ITEM_KEY THEN NULL 
				ELSE AV_NEW_MODULE_ITEM_KEY 
			END,
			CASE 
				WHEN LS_NEW_QNR_ID = LS_OLD_QNR_ID THEN 'Y' 
				ELSE 'N' 
			END
		);
    ...
    OPEN ATT_CUR;
    FETCH_LOOP: LOOP
        FETCH ATT_CUR INTO  LI_MODULE_ITEM_CODE, LI_MODULE_SUB_ITEM_CODE, LI_MODULE_SUB_ITEM_KEY, LS_FILE_ID, LS_FILE_NAME, LS_DESCRIPTION;
        ...
        SELECT 
            ...
            AV_UPDATE_USER
        FROM FB_FILE_DATA
        WHERE FB_FILE_DATA_ID = LS_FILE_ID;

        INSERT INTO FB_ATTACHMENTS(
            MODULE_ITEM_CODE,
            MODULE_ITEM_KEY,
            MODULE_SUB_ITEM_CODE,
            MODULE_SUB_ITEM_KEY,
