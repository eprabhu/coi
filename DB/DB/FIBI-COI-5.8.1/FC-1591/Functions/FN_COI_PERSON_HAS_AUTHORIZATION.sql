...
ELSEIF AV_MODULE_CODE = 23 THEN
	...
	WHERE T3.PERSON_ID = AV_LOGGED_IN_PERSON_ID)) OR ADMIN_PERSON_ID = AV_LOGGED_IN_PERSON_ID);
        ...
        WHERE T1.OPA_DISCLOSURE_ID = AV_MODULE_ITEM_KEY AND T1.ASSIGNEE_PERSON_ID = AV_LOGGED_IN_PERSON_ID AND T2.REVIEW_STATUS_CODE IN (4,7,8,9);
        IF LI_COUNT > 0 THEN
            RETURN LI_AUTHORIZED;
        END IF;
...
ELSEIF AV_MODULE_CODE = 28 THEN
        ...
        JOIN (SELECT ROLE_ID,RT.RIGHT_NAME 
                        ...
                        WHERE RT.RIGHT_NAME IN ( 'VIEW_MFTRP_DECLARATION')) RLE 
                ON RLE.ROLE_ID = PR.ROLE_ID
        WHERE PR.PERSON_ID = AV_LOGGED_IN_PERSON_ID
        AND ((PR. DESCEND_FLAG = 'Y' AND EXISTS (SELECT 1
                                                FROM UNIT_WITH_CHILDREN 
