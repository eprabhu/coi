...
ELSEIF AV_MODULE_CODE = 8 THEN
        ...
        'MANAGE_ENTITY_COMPLIANCE','VERIFY_ENTITY','ADD_APPROVER_WORKFLOW', 'BYPASS_WORKFLOW')) RLE
        WHERE PR.PERSON_ID = AV_PERSON_ID
          AND (( PR. DESCEND_FLAG = 'Y' AND EXISTS (SELECT 1
                                                                 FROM UNIT_WITH_CHILDREN 
                                                                                  WHERE UNIT_NUMBER = PR.UNIT_NUMBER
        ...
        (SELECT ROLE_ID,RT.RIGHT_NAME 
           FROM ROLE_RIGHTS RR,
            ...
            WHERE RR.RIGHT_ID = RT.RIGHT_ID
              ...
              'VIEW_OPA_DISCLOSURE_PRIVATE_ATTACHMENTS', 'ADD_APPROVER_WORKFLOW', 'BYPASS_WORKFLOW','VIEW_OPA_COMMENTS','MAINTAIN_OPA_COMMENTS',
              'OPA_DEPARTMENT_ADMINISTRATOR', 'MAINTAIN_OPA_REVIEWER')) RLE
        WHERE PR.PERSON_ID = AV_PERSON_ID
          AND (( PR. DESCEND_FLAG = 'Y' AND EXISTS (SELECT 1
                                                                 FROM UNIT_WITH_CHILDREN 
                                                                                  WHERE UNIT_NUMBER = PR.UNIT_NUMBER
...
SELECT DISTINCT RLE.RIGHT_NAME
 ...
 ELSEIF AV_MODULE_CODE = 28 THEN
	SELECT HOME_UNIT INTO GC_LEAD_UNIT FROM PERSON WHERE PERSON_ID = AV_LOGGED_IN_PERSON_ID;
