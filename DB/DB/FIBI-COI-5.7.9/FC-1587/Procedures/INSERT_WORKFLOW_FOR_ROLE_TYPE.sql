...
ELSEIF AV_ROLE_TYPE = 2301 THEN
    ...
    WITH RECURSIVE unit_hierarchy AS (
		 	SELECT UNIT_NUMBER, CHILD_UNIT_NUMBER, ROW_NUMBER() OVER (ORDER BY UNIT_NUMBER) AS ROW_NUM
		    FROM UNIT_WITH_CHILDREN
		...
		left join unit_hierarchy uh on uh.UNIT_NUMBER =  PR.UNIT_NUMBER and uh.UNIT_NUMBER != LS_LEAD_UNIT
		left join unit_hierarchy uh1 on uh1.UNIT_NUMBER = LS_LEAD_UNIT
		where( (PR.UNIT_NUMBER = LS_LEAD_UNIT)
        		OR
        			(PR.DESCEND_FLAG = 'Y' AND  uh.UNIT_NUMBER is not null and uh1.ROW_NUM = (uh.ROW_NUM + 1)   and NOT EXISTS (
        	            SELECT 1
...
ELSEIF AV_ROLE_TYPE = 52 THEN
    ...
    IF AV_MODULE_CODE = 23 THEN
		...
		LEFT JOIN DIRECT_ROLE_EXISTS DRE ON PR.ROLE_ID = DRE.ROLE_ID
		left join unit_hierarchy uh on uh.UNIT_NUMBER =  PR.UNIT_NUMBER and uh.UNIT_NUMBER != LS_LEAD_UNIT
        left join unit_hierarchy uh1 on uh1.UNIT_NUMBER = LS_LEAD_UNIT
        where( (PR.UNIT_NUMBER = LS_LEAD_UNIT)
        		OR
