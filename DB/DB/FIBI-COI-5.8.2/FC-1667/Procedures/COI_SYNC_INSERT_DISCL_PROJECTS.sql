        ...
        FROM (
			...
			SELECT DISTINCT MODULE_ITEM_KEY, MODULE_CODE, DISCLOSURE_VALIDATION_FLAG, DISCLOSURE_REQUIRED_FLAG,
				...
				WHERE T1.KEY_PERSON_ID = AV_PERSON_ID AND T1.PERSON_STATUS != 'I'
					...
					AND T1.AWARD_STATUS_CODE IN ('1','6','3','7')
					AND T4.FCOI_NEEDED = 'Y')T
			)
			SELECT MODULE_ITEM_KEY, MODULE_CODE
			FROM AwardRanks
			WHERE AwardRanks.MODULE_ITEM_KEY NOT IN (
						SELECT MODULE_ITEM_KEY
						FROM COI_DISCL_PROJECTS
						WHERE DISCLOSURE_ID = AV_DISCLOSURE_ID
						AND MODULE_CODE = 1
					)
			AND (DISCLOSURE_VALIDATION_FLAG = 'SELF'  
			...
			UNION
				...
				INNER JOIN COI_PROJECT_TYPE C4 ON C4.COI_PROJECT_TYPE_CODE = C3.COI_PROJECT_TYPE_CODE
                WHERE C2.PERSON_ID = AV_PERSON_ID AND C3.PERSON_STATUS <> 'I' AND C3.AWARD_STATUS_CODE IN ('1','6','3','7')
				AND C2.DISCLOSURE_ID <> AV_DISCLOSURE_ID AND C4.FCOI_NEEDED = 'Y' AND C3.AWARD_NUMBER NOT IN (
						SELECT MODULE_ITEM_KEY
						FROM COI_DISCL_PROJECTS
						WHERE DISCLOSURE_ID = AV_DISCLOSURE_ID
						AND MODULE_CODE = 1
