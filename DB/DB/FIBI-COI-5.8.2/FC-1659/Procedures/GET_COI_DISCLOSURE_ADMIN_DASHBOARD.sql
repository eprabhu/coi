...
CREATE PROCEDURE `GET_COI_DISCLOSURE_ADMIN_DASHBOARD`(
    ...
    AV_DISCLOSURE_PERSON               VARCHAR(90),
...
BEGIN
        ...
        IF AV_TAB_TYPE = 'ALL_REVIEWS' THEN
        	SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, ', ACCESS_TMP AS ( SELECT UNIT_NUMBER
							FROM PERSON_ROLES T1
							INNER JOIN ROLE_RIGHTS T2 ON T1.ROLE_ID = T2.ROLE_ID
							INNER JOIN RIGHTS T3 ON T2.RIGHT_ID = T3.RIGHT_ID 
							WHERE T1.DESCEND_FLAG = ''N'' AND T1.PERSON_ID = ''',AV_PERSON_ID,'''
        ...
        IF AV_TAB_TYPE = 'ALL_DISCLOSURES' THEN
                        ...
                        OR EXISTS (
							SELECT 1
							FROM DISCLOSURE_AFFILIATED_UNITS DAU
        ...
        ELSEIF AV_TAB_TYPE = 'NEW_SUBMISSIONS' THEN
        				...
        				SET LS_ADMIN_GROUP_FCOI_CONDITION = CONCAT(' AND (T1.HOME_UNIT IN (SELECT DISTINCT UNIT_NUMBER FROM FCOI_ACCESS_TMP) OR EXISTS (
							SELECT 1
							FROM DISCLOSURE_AFFILIATED_UNITS DAU
							WHERE DAU.DISCLOSURE_ID = T1.DISCLOSURE_ID
							  AND DAU.UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM FCOI_ACCESS_TMP)
        ...
        ELSEIF AV_TAB_TYPE = 'ALL_REVIEWS' THEN
        				...
        				SET TAB_QUERY = CONCAT(TAB_QUERY,' AND (T1.REVIEW_STATUS_CODE IN (4,7,8,9) AND T1.VERSION_STATUS IN (''PENDING'', ''ACTIVE'')
                    ...
                    OR EXISTS (
							...
							FROM DISCLOSURE_AFFILIATED_UNITS DAU
							WHERE DAU.DISCLOSURE_ID = T1.DISCLOSURE_ID
							  AND DAU.UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM ACCESS_TMP)
						)
                    )))');

        ELSEIF AV_TAB_TYPE = 'MY_REVIEWS' THEN

        				SET TAB_QUERY = CONCAT(TAB_QUERY,' AND T1.REVIEW_STATUS_CODE IN (3,7,8,9)
        ...
        END IF;
						...
						END IF;
							SET LS_FILTER_CONDITION = CONCAT(
								LS_FILTER_CONDITION,
								' (T.DISCLOSURE_PERSON_FULL_NAME LIKE ''', AV_DISCLOSURE_PERSON, 
								''' OR T.PERSON_ID LIKE ''', AV_DISCLOSURE_PERSON, ''') AND '
							);
						ELSE
							SET LS_FILTER_CONDITION = CONCAT(
								LS_FILTER_CONDITION,
								' T.PERSON_ID = ''', AV_DISCLOSURE_PERSON, ''' AND '
							);
						END IF;
					END IF;
						...
						END IF;
							SET LS_FILTER_CONDITION = CONCAT(
								LS_FILTER_CONDITION,
								' (T.ENTITY_NAMES LIKE ''', AV_ENTITY, 
								''' OR T.ENTITY_IDS LIKE ''', AV_ENTITY, ''') AND '
							);
						ELSE
							SET LS_FILTER_CONDITION = CONCAT(
								LS_FILTER_CONDITION,
								' FIND_IN_SET(''', AV_ENTITY, ''', REPLACE(T.ENTITY_IDS, '' '', '''')) AND '
							);
						END IF;
					END IF;
...
SELECT TRIM(TRAILING 'AND ' FROM LS_FILTER_CONDITION) INTO LS_FILTER_CONDITION FROM DUAL;
        END IF;
        	...
        	SET SELECTED_FIELD_LIST = CONCAT(
											...
											FN_PERSON_HAS_RIGHTS(''VIEW_COI_COMMENTS'',''',AV_PERSON_ID,''', NULL, 8, T1.DISCLOSURE_ID, ''RIGHT_IN_A_DOCUMENT'') AS CAN_VIEW_COMMENTS,
											FN_PERSON_HAS_RIGHTS(''MAINTAIN_COI_COMMENTS'',''',AV_PERSON_ID,''',NULL, 8, T1.DISCLOSURE_ID, ''RIGHT_IN_A_DOCUMENT'') AS CAN_MAINTAIN_COMMENTS,
											FN_PERSON_HAS_RIGHTS(''VIEW_COI_PRIVATE_COMMENTS'',''',AV_PERSON_ID,''', NULL, 8, T1.DISCLOSURE_ID, ''RIGHT_IN_A_DOCUMENT'') AS CAN_VIEW_PRIVATE_COMMENTS,
											FN_PERSON_HAS_RIGHTS(''MAINTAIN_COI_PRIVATE_COMMENTS'',''',AV_PERSON_ID,''', NULL, 8, T1.DISCLOSURE_ID, ''RIGHT_IN_A_DOCUMENT'') AS CAN_MAINTAIN_PRIVATE_COMMENTS,
											FN_PERSON_HAS_RIGHTS(''MAINTAIN_COI_RESOLVE_COMMENTS'',''',AV_PERSON_ID,''', NULL, 8, T1.DISCLOSURE_ID, ''RIGHT_IN_A_DOCUMENT'') AS CAN_RESOLVE_COMMENTS,
                                            CASE 
    ...
    END AS UNIT_ACCESS_TYPE'
        	...
        	SET JOIN_CONDITION = CONCAT(
        								...
        								LEFT JOIN (
        									...
        									GROUP_CONCAT(DISTINCT(T4.COUNTRY_CODE) SEPARATOR ",") AS ENTITY_COUNTRIES, GROUP_CONCAT(DISTINCT(T4.ENTITY_ID) SEPARATOR ", ") AS ENTITY_IDS
        									FROM COI_DISCL_PROJECTS T1
