...
SET SQL_SAFE_UPDATES = 0;
    ...
    SET NOT_FOUND = 0;
      ...
      IF LS_HOME_UNIT IS NOT NULL AND LS_HOME_UNIT<> '' THEN 
        ...
        IF LI_COUNT = 0 THEN 
            UPDATE FIBI_COI_PERSON SET VALIDATION_STATUS = 'ERROR', VALIDATION_MESSEGE = CONCAT('Person Home unit ''',LS_HOME_UNIT,''' is not Available in Unit Table') WHERE PERSON_ID = LS_PERSON_ID;
            ITERATE SEL_DATA;
        END IF;
	  END IF;
      ...
      IF LS_COUNTRY_OF_CITIZENSHIP IS NOT NULL AND LS_COUNTRY_OF_CITIZENSHIP <> '' THEN 
        ...
        IF LI_COUNT = 0 THEN 
            UPDATE FIBI_COI_PERSON SET VALIDATION_STATUS = 'ERROR', VALIDATION_MESSEGE = CONCAT('Person Country of Citizen Code ''',LS_COUNTRY_OF_CITIZENSHIP,''' is not Available in Country Table') WHERE PERSON_ID = LS_PERSON_ID;
            ITERATE SEL_DATA;
        END IF;
	...
	END IF;
        ...
        IF LI_COUNT = 0 THEN 
            UPDATE FIBI_COI_PERSON SET VALIDATION_STATUS = 'ERROR', VALIDATION_MESSEGE = CONCAT('Person Country Code ''',LS_COUNTRY_CODE,''' is not Available in Country Table') WHERE PERSON_ID = LS_PERSON_ID;
            ITERATE SEL_DATA;
        END IF;
		END IF;
	...
	SELECT 
		...
		T1.VISA_RENEWAL_DATE, (CASE WHEN T1.STATUS  = 'Y' THEN 'A' ELSE 'I' END), T1.SALARY_ANNIVERSARY_DATE, T1.UPDATE_TIMESTAMP,
		T1.UPDATE_USER
