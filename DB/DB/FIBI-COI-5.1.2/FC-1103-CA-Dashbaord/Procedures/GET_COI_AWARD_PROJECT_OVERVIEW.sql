...
DROP  TABLE if exists DisclosureAwardProjects;
	...
	END AS DISCLOSURE_STATUS
		,CASE 
    WHEN FN_CHK_HIERARCHY_HAS_DISCL(T1.AWARD_NUMBER, T1.KEY_PERSON_ID) <> 'NO DATA'
        THEN FN_SET_PROJECT_DISCLOSURE_REVIEW_STATUS(T1.AWARD_NUMBER, T1.KEY_PERSON_ID)
    ELSE CASE 
        WHEN T1.DISCLOSURE_REQUIRED_FLAG = 'REQUIRED'
            THEN T9.DESCRIPTION
        WHEN T1.DISCLOSURE_REQUIRED_FLAG = 'NOT_REQUIRED'
...
END AS PROJECT_DISCLOSURE_REVIEW_STATUS
    ...
    INNER  JOIN coi_int_stage_award_person CISA ON CISA.PROJECT_NUMBER = T1.AWARD_NUMBER
    WHERE CISA.NEW_DISCLOSURE_REQUIRED = 'Y'
    and T55.REVIEW_STATUS_CODE = 1
...
END AS PROJECT_DISCLOSURE_REVIEW_STATUS
	...
	INNER JOIN (
		...
		AND T55.PERSON_ID = T1.KEY_PERSON_ID
	INNER JOIN coi_review_status_type T9 ON T9.REVIEW_STATUS_CODE = T55.REVIEW_STATUS_CODE
    INNER JOIN coi_int_stage_award_person CISA ON CISA.PROJECT_NUMBER = T1.AWARD_NUMBER
    WHERE CISA.NEW_DISCLOSURE_REQUIRED <> 'Y'
    AND NOT EXISTS (SELECT 1 FROM coi_int_stage_award_person WHERE NEW_DISCLOSURE_REQUIRED = 'Y' AND PROJECT_NUMBER = T1.AWARD_NUMBER);
...
SELECT * FROM (SELECT 
    ...
    WHEN T1.PROJECT_REVIEW_STATUS =''DISCLOSURE_REVIEW_NA'' THEN ''N/A''
...
(SELECT FN_GET_OVAL_AWD_PRJT_DISCL_STATUS(T1.EXTERNAL_SYSTEM_REF_ID)) AS PROJECT_SUBMISSION_STATUS,
	...
	(SELECT PROJECT_DISCLOSURE_REVIEW_STATUS FROM DisclosureAwardProjects WHERE PROJECT_ID = T1.EXTERNAL_SYSTEM_REF_ID AND  KEY_PERSON_ID = T1.KEY_PERSON_ID LIMIT 1 ) AS REVIEW_STATUS,
     (CASE
    WHEN EXISTS (SELECT 1 FROM coi_int_stage_award_person WHERE PROJECT_NUMBER = T1.AWARD_NUMBER AND NEW_DISCLOSURE_REQUIRED = ''Y'') THEN ''Y''
    ELSE ''N''
  END) AS RESUBMISSION_FLAG
	...
	WHERE 1=1 ',IFNULL(LS_FILTER_CONDITION,''), ' GROUP BY T1.EXTERNAL_SYSTEM_REF_ID , T1.KEY_PERSON_ID,T1.KEY_PERSON_ROLE_CODE )T1',ifnull(LS_ORDER_BY,''),' )T ',IFNULL(LS_OFFSET_CONDITION,''),' ');
