...
END IF;
				...
				SET JOIN_CONDITION =  CONCAT(JOIN_CONDITION,' LEFT JOIN COI_DISCL_PROJECTS T9 ON T9.DISCLOSURE_ID = T1.DISCLOSURE_ID
										LEFT JOIN (SELECT PROPOSAL_NUMBER AS PROPOSAL_IDS,
												  TITLE  AS PROPOSAL_TITLES, PROPOSAL_NUMBER AS EXTERNAL_SYSTEM_REF_ID FROM COI_INT_STAGE_DEV_PROPOSAL ) T14 ON T14.EXTERNAL_SYSTEM_REF_ID = T9.MODULE_ITEM_KEY
										LEFT  JOIN (SELECT PROJECT_NUMBER AS AWARD_NUMBER, PROJECT_ID AS EXTERNAL_SYSTEM_REF_ID,
												   TITLE AS AWARD_TITLES FROM COI_INT_STAGE_AWARD )T15  ON T15.AWARD_NUMBER = T9.MODULE_ITEM_KEY
										LEFT  JOIN (SELECT PROJECT_NUMBER  AS IP_NUMBER, PROJECT_NUMBER AS EXTERNAL_SYSTEM_REF_ID,
												   TITLE AS IP_TITLE FROM COI_INT_STAGE_PROPOSAL )T16  ON T16.IP_NUMBER = T9.MODULE_ITEM_KEY ');
