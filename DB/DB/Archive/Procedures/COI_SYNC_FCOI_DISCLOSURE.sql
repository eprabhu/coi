...
CREATE PROCEDURE `COI_SYNC_FCOI_DISCLOSURE`(
	...
	DECLARE LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE 		        	VARCHAR(10);
        ...
        BEGIN
    		...
    		IF LI_FCOI_TYPE_CODE != 2 THEN
    		    ...
    		    -- Deletes the projects from a FCOI disclosure based on certain conditions below
					...
					WITH AwardRanks AS (
							...
							WHERE C2.PERSON_ID = AV_PERSON_ID AND C3.PERSON_STATUS <> 'I' AND C3.AWARD_STATUS_CODE IN ('1', '6', '3', '7')
							...
							LEFT JOIN (SELECT DISTINCT AW2.PROJECT_NUMBER FROM COI_INT_STAGE_AWARD AW1
										...
										WHERE AW2.PROJECT_STATUS_CODE IN ('1', '6', '3', '7') AND AW3.KEY_PERSON_ID = AV_PERSON_ID
							...
							WHERE T1.KEY_PERSON_ID = AV_PERSON_ID AND T1.PERSON_STATUS != 'I'
								...
								AND T1.AWARD_STATUS_CODE IN ('1', '6', '3', '7') -- Active/Hold/Pending
    		    ...
    		    COMMIT;
            END IF;
    		SELECT VALUE INTO LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE FROM PARAMETER WHERE PARAMETER_NAME = 'ENGAGEMENT_TYPE_FOR_COI_DISCLOSURE';
			
    		-- This single section handles ALL_ENG, ALL_FIN, and ALL_SFI cases using conditional logic
			
			START TRANSACTION;
			-- Updating the relation on person entity (SFI) versioning on project person entity relation table
			-- brings engagement types based on parameter set
			...
			UPDATE COI_DISCL_PROJECT_ENTITY_REL AS T
				...
				JOIN(SELECT DISTINCT T1.PERSON_ENTITY_ID AS PREVIOUS_PERSON_ENTITY_ID,T2.PERSON_ENTITY_ID, T1.PERSON_ENTITY_NUMBER
						...
						WHERE T2.VERSION_STATUS='ACTIVE' 
						  AND (
						      LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_ENG' 
						      OR (LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_FIN' AND T4.DISCLOSURE_TYPE_CODE = '1')
						      OR (LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_SFI' AND T2.IS_SIGNIFICANT_FIN_INTEREST = 'Y')
						  )
						  AND T1.PERSON_ENTITY_ID != T2.PERSON_ENTITY_ID
						  AND T.DISCLOSURE_ID = AV_DISCLOSURE_ID
					) AS T6 ON T6.PERSON_ENTITY_NUMBER = T.PERSON_ENTITY_NUMBER
				SET T.PREVIOUS_PERSON_ENTITY_ID = T6.PREVIOUS_PERSON_ENTITY_ID, 
				    T.PERSON_ENTITY_ID = T6.PERSON_ENTITY_ID, 
				    T.UPDATE_TIMESTAMP = utc_timestamp()
				WHERE T.PERSON_ENTITY_NUMBER = T6.PERSON_ENTITY_NUMBER AND T0.DISCLOSURE_ID = AV_DISCLOSURE_ID;
				
			COMMIT;
			
			START TRANSACTION;
			-- Updating the relation on person entity (SFI) versioning on Disclosure person entity relation table
			-- brings engagement types based on parameter set
			-- SFI should be active
			-- latest ID of SFI version will be updated on PERSON_ENTITY_ID column and previous will be updated on PREVIOUS_PERSON_ENTITY_ID
			UPDATE COI_DISCL_PERSON_ENTITY_REL AS T
				JOIN(SELECT DISTINCT T1.PERSON_ENTITY_ID AS PREVIOUS_PERSON_ENTITY_ID,T2.PERSON_ENTITY_ID, T1.PERSON_ENTITY_NUMBER
			...
			-- SFI should be active
			INSERT IGNORE INTO COI_DISCL_PROJECT_ENTITY_REL (`COI_DISCL_PROJECTS_ID`, `PERSON_ENTITY_ID`, `PERSON_ENTITY_NUMBER`, `ENTITY_ID`, `UPDATE_TIMESTAMP`, `UPDATED_BY`)
				...
				INNER JOIN (SELECT t1.PERSON_ENTITY_ID, t1.PERSON_ENTITY_NUMBER, t1.ENTITY_ID
					...
					WHERE t1.VERSION_STATUS = 'ACTIVE' 
					  AND (
					      LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_ENG' 
					      OR (LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_FIN' AND t3.DISCLOSURE_TYPE_CODE = '1')
					      OR (LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_SFI' AND t1.IS_SIGNIFICANT_FIN_INTEREST = 'Y')
					  )
			...
			START TRANSACTION;
			-- Inserting when a new engagement comes
			-- engagement types based on parameter set
			-- SFI should be active
			INSERT IGNORE INTO COI_DISCL_PERSON_ENTITY_REL (`DISCLOSURE_ID`, `PERSON_ENTITY_ID`, `PERSON_ENTITY_NUMBER`, `ENTITY_ID`, `UPDATE_TIMESTAMP`, `UPDATED_BY`)
				SELECT DISTINCT AV_DISCLOSURE_ID, T2.PERSON_ENTITY_ID, T2.PERSON_ENTITY_NUMBER, T2.ENTITY_ID, UTC_TIMESTAMP(), AV_PERSON_ID
			FROM (
				SELECT PE.PERSON_ENTITY_ID, PE.PERSON_ENTITY_NUMBER, PE.ENTITY_ID
				FROM PERSON_ENTITY PE
				INNER JOIN PERSON_ENTITY_RELATIONSHIP PER ON PER.PERSON_ENTITY_ID = PE.PERSON_ENTITY_ID
				INNER JOIN VALID_PERSON_ENTITY_REL_TYPE VPERT ON VPERT.VALID_PERS_ENTITY_REL_TYP_CODE = PER.VALID_PERS_ENTITY_REL_TYP_CODE
				WHERE PE.VERSION_STATUS = 'ACTIVE' 
				  AND (
				      LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_ENG' 
				      OR (LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_FIN' AND VPERT.DISCLOSURE_TYPE_CODE = '1')
				      OR (LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_SFI' AND PE.IS_SIGNIFICANT_FIN_INTEREST = 'Y')
				  )
				  AND PE.PERSON_ID = AV_PERSON_ID
			) T2
			WHERE NOT EXISTS (
				SELECT 1 
				FROM COI_DISCL_PERSON_ENTITY_REL REL
				WHERE REL.DISCLOSURE_ID = AV_DISCLOSURE_ID
				  AND REL.PERSON_ENTITY_ID = T2.PERSON_ENTITY_ID
			);
			COMMIT;
			...
			DELETE T4, T2, T FROM COI_DISCL_PROJECT_ENTITY_REL T
				INNER JOIN COI_DISCL_PROJECTS T0 ON T0.COI_DISCL_PROJECTS_ID = T.COI_DISCL_PROJECTS_ID
				LEFT JOIN  DISCL_COMMENT T2 ON 
				     (T2.COMPONENT_TYPE_CODE = '1' OR T2.COMPONENT_TYPE_CODE = '6') AND T2.MODULE_ITEM_KEY = T0.DISCLOSURE_ID
			         AND T2.SUB_MODULE_ITEM_KEY = T.COI_DISCL_PROJECT_ENTITY_REL_ID 
				INNER JOIN (
					...
					WHERE (T2.VERSION_STATUS='INACTIVE' OR T3.PERSON_ENTITY_ID IS NULL
						OR (LS_ENGAGEMENT_TYPE_FOR_DISCLOSURE = 'ALL_FIN' AND NOT EXISTS (
			...
			-- Deleting an SFI relation from disclosure person entity relationship table when it is Inactivated
			DELETE T FROM COI_DISCL_PERSON_ENTITY_REL T
				INNER JOIN (
					SELECT DISTINCT T2.PERSON_ENTITY_ID, T1.PERSON_ENTITY_NUMBER
					FROM COI_DISCL_PERSON_ENTITY_REL T1
					INNER JOIN PERSON_ENTITY T2 ON T2.PERSON_ENTITY_NUMBER = T1.PERSON_ENTITY_NUMBER
					...
					AND T2.VERSION_STATUS != 'ARCHIVE' AND T1.DISCLOSURE_ID = AV_DISCLOSURE_ID
				) AS T3 ON T3.PERSON_ENTITY_NUMBER = T.PERSON_ENTITY_NUMBER
