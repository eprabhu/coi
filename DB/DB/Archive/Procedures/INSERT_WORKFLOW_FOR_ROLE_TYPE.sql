...
IF AV_MODULE_CODE IN(1, 14, 16) THEN -- NTU
...
ELSEIF AV_ROLE_TYPE = 69 then
	...
	SELECT GROUP_CONCAT(DISTINCT T1.PERSON_ID) INTO LI_TEMP_PERSON_ID FROM PERSON_ROLES T1 INNER JOIN ADMIN_GROUP T2 ON T2.ROLE_ID = T1.ROLE_ID WHERE T2.MODULE_CODE=23;
...
ELSEIF AV_ROLE_TYPE = 2301 THEN
        ...
        WITH RECURSIVE unit_hierarchy AS (
            SELECT t1.UNIT_NUMBER, t1.CHILD_UNIT_NUMBER, 
                   CASE 
                       WHEN u.PARENT_UNIT_NUMBER IS NULL THEN 1
                       ELSE FN_GET_UNIT_HIERARCHY_LEVEL(t1.UNIT_NUMBER)
                   END AS LEVEL
            FROM UNIT_WITH_CHILDREN t1
            JOIN UNIT u ON t1.UNIT_NUMBER = u.UNIT_NUMBER
            WHERE t1.CHILD_UNIT_NUMBER = LS_LEAD_UNIT AND t1.UNIT_NUMBER != LS_LEAD_UNIT
        ),
        first_matched_unit AS (
            SELECT uh.UNIT_NUMBER
            FROM unit_hierarchy uh
            JOIN PERSON_ROLES pr ON uh.UNIT_NUMBER = pr.UNIT_NUMBER
            WHERE pr.DESCEND_FLAG = 'Y' AND pr.ROLE_ID = 1374
            ORDER BY uh.LEVEL DESC
            LIMIT 1
        )
        SELECT GROUP_CONCAT(DISTINCT PR.PERSON_ID SEPARATOR ',') INTO LI_TEMP_PERSON_ID
        ...
        LEFT JOIN first_matched_unit fmu ON PR.UNIT_NUMBER = fmu.UNIT_NUMBER
        WHERE P.STATUS = 'A' AND (
            PR.UNIT_NUMBER = LS_LEAD_UNIT or (fmu.UNIT_NUMBER IS NOT NULL AND
                NOT EXISTS (
                    SELECT 1
                    FROM PERSON_ROLES PR2
                    WHERE PR2.UNIT_NUMBER = LS_LEAD_UNIT
                    AND PR2.ROLE_ID = PR.ROLE_ID
                )
            )
        );
...
ELSEIF AV_ROLE_TYPE = 2302 THEN
        ...
        first_matched_unit AS (
            ...
            FROM unit_hierarchy uh
            JOIN PERSON_ROLES pr ON uh.UNIT_NUMBER = pr.UNIT_NUMBER
            WHERE pr.DESCEND_FLAG = 'Y' AND pr.ROLE_ID = 1375
            ORDER BY uh.LEVEL DESC
            LIMIT 1
        )
        SELECT GROUP_CONCAT(DISTINCT PR.PERSON_ID SEPARATOR ',') INTO LI_TEMP_PERSON_ID
...
ELSEIF AV_ROLE_TYPE = 52 THEN
    ...
    IF AV_MODULE_CODE = 23 THEN
        ...
        first_matched_unit AS (
            ...
            FROM unit_hierarchy uh
            JOIN PERSON_ROLES pr ON uh.UNIT_NUMBER = pr.UNIT_NUMBER
            WHERE pr.DESCEND_FLAG = 'Y' AND pr.ROLE_ID = 1338
            ORDER BY uh.LEVEL DESC
            LIMIT 1
        )
        SELECT GROUP_CONCAT(DISTINCT PR.PERSON_ID SEPARATOR ',') INTO LI_TEMP_PERSON_ID
