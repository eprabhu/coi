DELIMITER //
CREATE FUNCTION FN_CHK_ACTIVE_PROJECTS(
    AV_PERSON_ID VARCHAR(40)
) RETURNS int
    READS SQL DATA
    DETERMINISTIC
BEGIN
    DECLARE LI_ACTIVE_COUNT INT;

    WITH AWARD_RANK AS (
        SELECT DISTINCT MODULE_ITEM_KEY, MODULE_CODE, DISCLOSURE_VALIDATION_FLAG, DISCLOSURE_REQUIRED_FLAG,
            ROW_NUMBER() OVER (
                PARTITION BY SUBSTRING_INDEX(MODULE_ITEM_KEY, '-', 1)  
                ORDER BY 
                    CASE 
                        ...
                        ELSE 3
                    END,
                    CAST(SUBSTRING_INDEX(MODULE_ITEM_KEY, '-', -1) AS UNSIGNED)
            ) AS rn
        FROM (
            SELECT DISTINCT MODULE_ITEM_KEY, 1 AS MODULE_CODE, DISCLOSURE_VALIDATION_FLAG, DISCLOSURE_REQUIRED_FLAG
            FROM COI_DISCL_PROJECTS C1
            ...
            LEFT JOIN (
                SELECT DISTINCT AW2.PROJECT_NUMBER
                ...
                WHERE AW2.PROJECT_STATUS_CODE IN (1, 6, 3)
                    ...
                    AND NOT EXISTS (
                        SELECT 1 FROM COI_INT_STAGE_AWARD ROOT
                        WHERE ROOT.PROJECT_NUMBER = AW1.ROOT_PROJECT_NUMBER AND ROOT.PROJECT_STATUS_CODE = 5
                    )
            ) T5 ON T5.PROJECT_NUMBER = T1.AWARD_NUMBER
            WHERE T1.KEY_PERSON_ID = AV_PERSON_ID AND T1.PERSON_STATUS != 'I'
                AND (
                    T1.DISCLOSURE_VALIDATION_FLAG = 'SELF'
                    OR (
                        T1.DISCLOSURE_REQUIRED_FLAG = 'REQUIRED'
    ...
    SELECT COUNT(*) INTO LI_ACTIVE_COUNT
    FROM (
        SELECT MODULE_ITEM_KEY, MODULE_CODE
        FROM AWARD_RANK
        WHERE 
            DISCLOSURE_VALIDATION_FLAG = 'SELF'
            OR (
                DISCLOSURE_REQUIRED_FLAG = 'REQUIRED'
                ...
                AND NOT EXISTS (
                    ...
                    AND SUBSTRING_INDEX(AR.MODULE_ITEM_KEY, '-', 1) = SUBSTRING_INDEX(AWARD_RANK.MODULE_ITEM_KEY, '-', 1)
                )
            )
        
        UNION
        
        SELECT DISTINCT C1.MODULE_ITEM_KEY, 1
        FROM COI_DISCL_PROJECTS C1
        INNER JOIN COI_DISCLOSURE C2 ON C2.DISCLOSURE_ID = C1.DISCLOSURE_ID
        INNER JOIN COI_PROJECT_AWARD_V C3 ON C3.AWARD_NUMBER = C1.MODULE_ITEM_KEY
        INNER JOIN COI_PROJECT_TYPE C4 ON C4.COI_PROJECT_TYPE_CODE = C3.COI_PROJECT_TYPE_CODE
        ...
        SELECT T1.EXTERNAL_SYSTEM_REF_ID, 3
        FROM COI_PROJECT_PROPOSAL_V T1
        INNER JOIN COI_PROJECT_TYPE T4 ON T4.COI_PROJECT_TYPE_CODE = T1.COI_PROJECT_TYPE_CODE
        WHERE T1.KEY_PERSON_ID = AV_PERSON_ID AND T4.FCOI_NEEDED = 'Y'

        UNION

        SELECT T1.PROPOSAL_NUMBER, 2
        ...
        WHERE T1.KEY_PERSON_ID = AV_PERSON_ID AND T1.PERSON_STATUS != 'I'
            ...
            AND (
                (T2.KEY_PERSON_DISCL_REQUIREMENT = 'ALL' OR (T2.KEY_PERSON_DISCL_REQUIREMENT = 'PI-COI' AND T1.KEY_PERSON_ROLE_CODE IN (1,3)))
                OR (T3.KEY_PERSON_DISCL_REQUIREMENT = 'ALL' OR (T3.KEY_PERSON_DISCL_REQUIREMENT = 'PI-COI' AND T1.KEY_PERSON_ROLE_CODE IN (1,3)))
                OR (
                    (T1.PCK = 'PC' AND T1.KEY_PERSON_ROLE_CODE IN (1,3))
