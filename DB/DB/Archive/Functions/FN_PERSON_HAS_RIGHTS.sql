...
IF AV_PERMISSION_TYPE IN ('RIGHT_IN_A_DOCUMENT','RIGHT_IN_DEPT') THEN
        ...
        IF AV_MODULE_CODE IN (8, 23, 24, 28) THEN 
        	
        	IF AV_MODULE_CODE = 8 THEN
        
        		SELECT PERSON_ID INTO LS_PERSON_ID FROM COI_DISCLOSURE WHERE DISLCOSURE_ID = AV_MODULE_ITEM_KEY;

        	ELSEIF AV_MODULE_CODE = 23 THEN
        
        		SELECT PERSON_ID INTO LS_PERSON_ID FROM OPA_DISCLOSURE WHERE OPA_DISCLOSURE_ID = AV_MODULE_ITEM_KEY;

        	ELSEIF AV_MODULE_CODE = 24 THEN
        
        		SELECT PERSON_ID INTO LS_PERSON_ID FROM COI_TRAVEL_DISCLOSURE WHERE TRAVEL_DISCLOSURE_ID = AV_MODULE_ITEM_KEY;

        	ELSEIF AV_MODULE_CODE = 28 THEN
        
        		SELECT PERSON_ID INTO LS_PERSON_ID FROM COI_DECLARATION WHERE DECLARATION_ID = AV_MODULE_ITEM_KEY;
			  ...
			  AND (
					(PR.DESCEND_FLAG = 'Y' AND EXISTS (
						 ...
						 JOIN (
							  ...
							  AND IS_ACTIVE = 'Y'
							  UNION
							  SELECT HOME_UNIT AS UNIT_NUMBER
							  FROM COI_DISCLOSURE 
							  WHERE DISCLOSURE_ID = AV_MODULE_ITEM_KEY
						 ) DU ON UWC.CHILD_UNIT_NUMBER = DU.UNIT_NUMBER
						 WHERE UWC.UNIT_NUMBER = PR.UNIT_NUMBER
					))
				 OR
					(PR.DESCEND_FLAG = 'N' AND EXISTS (
						 SELECT 1
						 FROM (
							  SELECT UNIT_NUMBER 
							  FROM PERSON_AFFILIATED_UNITS 
						 ...
						 WHERE DU.UNIT_NUMBER = PR.UNIT_NUMBER
					))
				  );
