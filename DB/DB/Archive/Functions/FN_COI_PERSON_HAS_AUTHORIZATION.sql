...
*/
				...
				SELECT COUNT(*) INTO LI_COUNT FROM COI_DISCL_PROJECT_ENTITY_REL CDPER
				INNER JOIN COI_DISCL_PROJECTS CDP ON CDP.COI_DISCL_PROJECTS_ID = CDPER.COI_DISCL_PROJECTS_ID
				INNER JOIN COI_REVIEW CR ON CR.DISCLOSURE_ID = CDP.DISCLOSURE_ID
				WHERE CR.ASSIGNEE_PERSON_ID = AV_LOGGED_IN_PERSON_ID AND CDPER.PERSON_ENTITY_ID = AV_MODULE_ITEM_KEY; --  AND CR.REVIEW_STATUS_TYPE_CODE <> 2

				IF LI_COUNT > 0 THEN
                    RETURN LI_AUTHORIZED;
                END IF;

				SELECT COUNT(*) INTO LI_COUNT FROM OPA_DISCL_PERSON_ENTITY_REL ODPER
                INNER JOIN OPA_REVIEW O ON O.OPA_DISCLOSURE_ID = ODPER.DISCLOSURE_ID
				WHERE O.ASSIGNEE_PERSON_ID = AV_LOGGED_IN_PERSON_ID AND ODPER.PERSON_ENTITY_ID = AV_MODULE_ITEM_KEY; --  AND O.REVIEW_STATUS_TYPE_CODE <> 3
		...
		SELECT HOME_UNIT, REVIEW_STATUS_CODE, PERSON_ID INTO LS_LEAD_UNIT, LS_STATUS_CODE, LS_PERSON_ID FROM COI_DISCLOSURE WHERE DISCLOSURE_ID = AV_MODULE_ITEM_KEY;
        ...
        SELECT COUNT(1) 
			...
			JOIN (
				...
				WHERE RT.RIGHT_NAME IN (
					'MANAGE_FCOI_DISCLOSURE','VIEW_FCOI_DISCLOSURE',
					'MANAGE_PROJECT_DISCLOSURE','VIEW_PROJECT_DISCLOSURE',
					'MANAGE_TRAVEL_DISCLOSURE','VIEW_TRAVEL_DISCLOSURE',
					'VIEW_ADMIN_GROUP_COI'
				)
			) RLE 
			  ...
			  AND (
				(
				  ...
				  AND EXISTS (
					...
					WHERE UNIT_NUMBER = PR.UNIT_NUMBER
					  AND (
						   CHILD_UNIT_NUMBER = LS_LEAD_UNIT
						   OR CHILD_UNIT_NUMBER IN (
							   SELECT UNIT_NUMBER 
							   FROM PERSON_AFFILIATED_UNITS
							   WHERE PERSON_ID = LS_PERSON_ID
							   AND AFFILIATION_TYPE_CODE = '1'
							   AND IS_ACTIVE = 'Y'
						   )
					  )
				  )
				...
				OR (
				  PR.DESCEND_FLAG = 'N'
				  AND (
					   PR.UNIT_NUMBER = LS_LEAD_UNIT
					   OR PR.UNIT_NUMBER IN (
						   SELECT UNIT_NUMBER 
						   FROM PERSON_AFFILIATED_UNITS
						   WHERE PERSON_ID = LS_PERSON_ID
						   AND AFFILIATION_TYPE_CODE = '1'
...
ELSEIF AV_MODULE_CODE = 23 THEN
	...
	SELECT HOME_UNIT, REVIEW_STATUS_CODE, PERSON_ID INTO LS_LEAD_UNIT, LS_STATUS_CODE, LS_PERSON_ID FROM OPA_DISCLOSURE WHERE OPA_DISCLOSURE_ID = AV_MODULE_ITEM_KEY;
	...
	WHERE T3.PERSON_ID = AV_LOGGED_IN_PERSON_ID)) OR ADMIN_PERSON_ID = AV_LOGGED_IN_PERSON_ID);
        ...
        WHERE T1.OPA_DISCLOSURE_ID = AV_MODULE_ITEM_KEY AND T1.ASSIGNEE_PERSON_ID = AV_LOGGED_IN_PERSON_ID;
        IF LI_COUNT > 0 THEN
            RETURN LI_AUTHORIZED;
        END IF;

        SELECT COUNT(WF.MODULE_ITEM_ID) INTO  LI_COUNT FROM WORKFLOW WF
            INNER JOIN WORKFLOW_DETAIL WFD ON WF.WORKFLOW_ID = WFD.WORKFLOW_ID
            WHERE WF.MODULE_ITEM_ID = AV_MODULE_ITEM_KEY
            AND WFD.APPROVER_PERSON_ID = AV_LOGGED_IN_PERSON_ID
...
ELSEIF AV_MODULE_CODE = 24 THEN
	...
	SELECT HOME_UNIT, REVIEW_STATUS_CODE, PERSON_ID INTO LS_LEAD_UNIT, LS_STATUS_CODE, LS_PERSON_ID FROM COI_TRAVEL_DISCLOSURE WHERE TRAVEL_DISCLOSURE_ID = AV_MODULE_ITEM_KEY;
...
ELSEIF AV_MODULE_CODE = 27 THEN
	...
	WHERE T3.PERSON_ID = AV_LOGGED_IN_PERSON_ID)) OR ADMIN_PERSON_ID = AV_LOGGED_IN_PERSON_ID);
        ...
        IF LI_COUNT > 0 THEN
            RETURN LI_AUTHORIZED;
        END IF;
ELSEIF AV_MODULE_CODE = 28 THEN
        SELECT P.HOME_UNIT, CD.PERSON_ID INTO LS_LEAD_UNIT, LS_PERSON_ID 
        FROM PERSON P 
        JOIN COI_DECLARATION CD ON CD.PERSON_ID = P.PERSON_ID
        WHERE CD.DECLARATION_ID = AV_MODULE_ITEM_KEY;
        
        SELECT COUNT(*) INTO LI_COUNT FROM COI_DECLARATION WHERE DECLARATION_ID = AV_MODULE_ITEM_KEY AND PERSON_ID = AV_LOGGED_IN_PERSON_ID;
        IF LI_COUNT > 0 THEN
                RETURN LI_AUTHORIZED;
        END IF;
        ...
        JOIN (SELECT ROLE_ID,RT.RIGHT_NAME 
                        ...
                        WHERE RT.RIGHT_NAME IN ( 'VIEW_MFTRP_DECLARATION', 'MANAGE_MFTRP_DECLARATION')) RLE
                ON RLE.ROLE_ID = PR.ROLE_ID
        WHERE PR.PERSON_ID = AV_LOGGED_IN_PERSON_ID
        AND (
			    (
        ...
        SELECT COUNT(DECLARATION_ID) INTO  LI_COUNT FROM COI_DECLARATION WHERE DECLARATION_ID = AV_MODULE_ITEM_KEY AND (ADMIN_GROUP_ID IN(SELECT ADMIN_GROUP_ID FROM ADMIN_GROUP
        WHERE ROLE_ID IN (SELECT T1.ROLE_ID FROM ROLE_RIGHTS T1 INNER JOIN PERSON_ROLES T3 ON T1.ROLE_ID = T3.ROLE_ID
        WHERE T3.PERSON_ID = AV_LOGGED_IN_PERSON_ID)) OR ADMIN_PERSON_ID = AV_LOGGED_IN_PERSON_ID);
        ...
        IF LI_COUNT > 0 THEN
            RETURN LI_AUTHORIZED;
        END IF;
ELSEIF AV_MODULE_CODE = 29 THEN
        SELECT P.HOME_UNIT, CMP.PERSON_ID INTO LS_LEAD_UNIT, LS_PERSON_ID 
        FROM PERSON P 
        JOIN COI_MANAGEMENT_PLAN CMP ON CMP.PERSON_ID = P.PERSON_ID
        WHERE CMP.CMP_ID = AV_MODULE_ITEM_KEY;

        SELECT COUNT(*) INTO LI_COUNT FROM COI_MANAGEMENT_PLAN WHERE CMP_ID = AV_MODULE_ITEM_KEY AND CREATED_BY = AV_LOGGED_IN_PERSON_ID;
        IF LI_COUNT > 0 THEN
                RETURN LI_AUTHORIZED;
        END IF;

		SELECT COUNT(*) INTO LI_COUNT FROM COI_MANAGEMENT_PLAN WHERE CMP_ID = AV_MODULE_ITEM_KEY AND PERSON_ID = AV_LOGGED_IN_PERSON_ID;
        IF LI_COUNT > 0 THEN
                RETURN LI_AUTHORIZED;
        END IF;
        ...
        JOIN (SELECT ROLE_ID,RT.RIGHT_NAME 
                        FROM ROLE_RIGHTS RR
                        JOIN RIGHTS RT ON RR.RIGHT_ID = RT.RIGHT_ID
                        WHERE RT.RIGHT_NAME IN ( 'VIEW_CMP', 'MAINTAIN_CMP')) RLE
                ON RLE.ROLE_ID = PR.ROLE_ID
        WHERE PR.PERSON_ID = AV_LOGGED_IN_PERSON_ID
        AND (
			    (
        ...
        SELECT COUNT(T1.CMP_ID) INTO  LI_COUNT
        FROM COI_CMP_REVIEW T1
        INNER JOIN COI_MANAGEMENT_PLAN T2
        ON T1.CMP_ID = T2.CMP_ID
        WHERE T1.CMP_ID = AV_MODULE_ITEM_KEY AND T1.ASSIGNEE_PERSON_ID = AV_LOGGED_IN_PERSON_ID;
        IF LI_COUNT > 0 THEN
            RETURN LI_AUTHORIZED;
        END IF;

	SELECT COUNT(T1.TASK_ID) INTO  LI_COUNT
        FROM cmp_task T1
        INNER JOIN COI_MANAGEMENT_PLAN T2
        ON T1.CMP_ID = T2.CMP_ID
        WHERE T1.CMP_ID = AV_MODULE_ITEM_KEY AND T1.ASSIGNEE_PERSON_ID = AV_LOGGED_IN_PERSON_ID;
