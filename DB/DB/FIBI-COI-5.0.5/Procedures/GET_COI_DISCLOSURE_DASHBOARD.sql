...
END IF;
				...
				SET JOIN_CONDITION =  CONCAT(JOIN_CONDITION,' LEFT JOIN (SELECT IFNULL(COUNT(DISTINCT T2.PERSON_ENTITY_ID),0) AS NO_OF_SFI,T3.DISCLOSURE_ID FROM PERSON_ENTITY T1
											INNER JOIN COI_DISCL_PROJECT_ENTITY_REL T2 ON T2.PERSON_ENTITY_ID= T1.PERSON_ENTITY_ID
											INNER JOIN COI_DISCL_PROJECTS T3 ON T3.COI_DISCL_PROJECTS_ID = T2.COI_DISCL_PROJECTS_ID
											GROUP BY T3.DISCLOSURE_ID) T7 ON T7.DISCLOSURE_ID = T1.DISCLOSURE_ID
										...
										LEFT  JOIN (SELECT PROPOSAL_NUMBER AS IP_NUMBER , EXTERNAL_SYSTEM_REF_ID,
												   ...
												   ON T62.IP_NUMBER = T61.MODULE_ITEM_KEY  ');
...
IF AV_TAB_TYPE = 'CONSULTING_DISCLOSURES' THEN
		...
		IF AV_SEARCHWORD IS NOT NULL THEN
            ...
            SET SEARCH_CONDITION = CONCAT(' AND (T50.DISCLOSURE_ID LIKE ',AV_SEARCHWORD, ' OR T54.ENTITY_NAME LIKE ',AV_SEARCHWORD, ' OR
...
ELSE
		IF AV_SEARCHWORD IS NOT NULL THEN
			...
			T20.UNIT_NAME LIKE ', AV_SEARCHWORD, ' OR T20.UNIT_NUMBER LIKE ', AV_SEARCHWORD, ' OR T14.PROPOSAL_TITLE LIKE ', AV_SEARCHWORD, '  OR T15.AWARD_TITLE LIKE ', AV_SEARCHWORD, 'OR T62.IP_TITLE LIKE ', AV_SEARCHWORD,
        ...
        SET LS_DYN_SQL =CONCAT('SELECT DISTINCT *  FROM(SELECT DISTINCT
										...
										CASE WHEN T1.FCOI_TYPE_CODE = 2 THEN COALESCE(T14.PROPOSAL_ID,
											T62.IP_NUMBER,
											T15.AWARD_NUMBER) ELSE NULL END AS PROJECT_NUMBER,
										T63.BADGE_COLOR,
										T63.PROJECT_ICON,
