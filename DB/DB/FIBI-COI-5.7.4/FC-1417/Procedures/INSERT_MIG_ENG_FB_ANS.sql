...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        SELECT QH.QUESTIONNAIRE_ID 
        INTO LI_QUESTNR_ID 
        FROM QUEST_HEADER QH
        WHERE QH.QUESTIONNAIRE = 'Engagement questionnaire' 
            AND QH.IS_FINAL = 'Y' 
            AND QH.VERSION_NUMBER = (
                SELECT MAX(VERSION_NUMBER) 
                FROM QUEST_HEADER QH1
                WHERE QH1.QUESTIONNAIRE_NUMBER = QH.QUESTIONNAIRE_NUMBER
            );    
