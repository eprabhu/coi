...
BEGIN
    ...
    SELECT LOWER(TRIM(LS_ENTITY_NAME)) INTO LS_ENTITY_NAME
    ...
    SET LS_CLEANED = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LS_CLEANED, '&', ' '), '-', ' '), '/', ' '), '(', ' '), ')', ' ');
	...
	SET FILTER_CONDITION = CONCAT(' OR LOWER(e.PRIMARY_NAME) LIKE ''', LS_CLEANED, '''');
    ...
    WHERE e.ENTITY_STATUS_TYPE_CODE != 3
      ...
      AND (
			...
			, FILTER_CONDITION, ')
    ORDER BY e.PRIMARY_NAME ASC LIMIT 20');
