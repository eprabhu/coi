...
END IF;
    ...
    ELSEIF AV_TAB_TYPE = 'ALL_REVIEWS' THEN
        ...
        SET LS_FILTER_CONDITION = CONCAT(' T1.REVIEW_STATUS_CODE IN (3,7,8) ');
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION, ' AND ( T1.ADMIN_PERSON_ID = ''', AV_PERSON_ID ,'''
                    OR (T1.HOME_UNIT IN (SELECT DISTINCT UNIT_NUMBER FROM RW_ACCESS_TMP)))');

    ELSEIF AV_TAB_TYPE = 'ALL_DISCLOSURES' THEN

        SET LS_FILTER_CONDITION = CONCAT(' T1.REVIEW_STATUS_CODE != 1 ');

        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION, ' AND (T1.HOME_UNIT IN (SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP) OR T1.ADMIN_PERSON_ID = ''', AV_PERSON_ID ,''')  ');	
    END IF;
    ...
    /* --------Count or select conditions and select fields---------- */
