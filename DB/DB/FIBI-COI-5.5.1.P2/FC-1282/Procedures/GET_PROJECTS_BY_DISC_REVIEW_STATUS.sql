...
BEGIN
    WITH ProjectDisclosureDetails AS (
        ...
        LEFT JOIN (
            SELECT EXTERNAL_SYSTEM_REF_ID AS PROPOSAL_ID, KEY_PERSON_ID,TITLE as PROPOSAL_TITLE,PI_NAME as PROPOSAL_PI, LEAD_UNIT_NUMBER, LEAD_UNIT_NAME
            FROM COI_PROJECT_PROPOSAL_V WHERE PERSON_STATUS = 'A' AND DISCLOSURE_REQUIRED_FLAG <> 'NOT_REQUIRED'
        ) t5 ON t5.PROPOSAL_ID = t9.MODULE_ITEM_KEY AND t9.MODULE_CODE = 3
        LEFT JOIN (
            SELECT AWARD_NUMBER, KEY_PERSON_ID, TITLE as AWARD_TITLE, PI_NAME as AWARD_PI, LEAD_UNIT_NUMBER, LEAD_UNIT_NAME   
            FROM COI_PROJECT_AWARD_V WHERE PERSON_STATUS = 'A' AND DISCLOSURE_REQUIRED_FLAG <> 'NOT_REQUIRED'
    ...
    SELECT distinct
        ...
        T1.DEPARTMENT_NAME,
        NULL AS PROJECT_SUBMISSION_STATUS,
        NULL AS PROJECT_OVERALL_REVIEW_STATUS
