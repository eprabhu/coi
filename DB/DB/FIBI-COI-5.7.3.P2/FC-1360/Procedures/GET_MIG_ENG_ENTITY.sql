...
SELECT ENTITY_NAME INTO LS_ENTITY_NAME
...
WHERE e.ENTITY_STATUS_TYPE_CODE != 3
    ...
    AND (
        LOWER(TRIM(e.PRIMARY_NAME)) LIKE LOWER(LS_ENTITY_NAME)
        OR SOUNDEX(e.PRIMARY_NAME) LIKE SOUNDEX(SUBSTRING_INDEX(TRIM(LS_ENTITY_NAME), ' ', 1))
        OR LOWER(e.PRIMARY_NAME) LIKE CONCAT('%', SUBSTRING_INDEX(LOWER(TRIM(LS_ENTITY_NAME)), ' ', 1), '%')
    );
END
//
