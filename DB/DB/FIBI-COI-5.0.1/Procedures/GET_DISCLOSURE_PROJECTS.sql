...
DECLARE LS_PERSON_ID VARCHAR(40);
        ...
        SELECT DISTINCT
            ...
            LEFT JOIN (SELECT DISCLOSURE_DETAILS_ID , MODULE_ITEM_KEY,
						...
						WHERE T5.DISCLOSURE_ID= AV_DISCLOSURE_ID AND MODULE_CODE = 1
						GROUP BY MODULE_ITEM_KEY, MODULE_CODE )T4 ON T4.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
		...
		T2.LEAD_UNIT_NAME AS HOME_UNIT_NAME,
            ...
            INNER JOIN EPS_PROP_PERSON_ROLE T3 ON T2.KEY_PERSON_ROLE_CODE = T3.PROP_PERSON_ROLE_ID
            LEFT JOIN (SELECT DISCLOSURE_DETAILS_ID ,
						MODULE_ITEM_KEY,
						MODULE_CODE,
						MAX(T5.PROJECT_CONFLICT_STATUS_CODE) AS PROJECT_CONFLICT_STATUS_CODE
						FROM COI_DISCL_ENT_PROJ_DETAILS T5
			 ...
			 T2.PROPOSAL_START_DATE AS START_DATE,
					...
					T2.LEAD_UNIT_NAME AS HOME_UNIT_NAME,
                        ...
                        LEFT JOIN (SELECT DISCLOSURE_DETAILS_ID ,
									...
									MAX(T5.PROJECT_CONFLICT_STATUS_CODE) AS PROJECT_CONFLICT_STATUS_CODE
									FROM COI_DISCL_ENT_PROJ_DETAILS T5
									WHERE T5.DISCLOSURE_ID= AV_DISCLOSURE_ID AND MODULE_CODE = 2 GROUP BY MODULE_ITEM_KEY, MODULE_CODE )T4 ON T4.MODULE_ITEM_KEY = T2.EXTERNAL_SYSTEM_REF_ID
                        LEFT JOIN (SELECT MODULE_ITEM_KEY,
									MODULE_CODE,
									COUNT(T5.PERSON_ENTITY_ID) AS RELATIONSHIP_COUNT
                                    FROM COI_DISCL_ENT_PROJ_DETAILS T5
