...
SET LS_OFFSET = (AV_LIMIT * AV_PAGED);
    ...
    SET LS_JOIN_CONDITION = CONCAT(LS_JOIN_CONDITION, ' INNER JOIN PERSON T2 ON T1.KEY_PERSON_ID = T2.PERSON_ID
														...
														LEFT JOIN COI_DISCLOSURE T6 ON T6.MODULE_ITEM_KEY = T1.EXTERNAL_SYSTEM_REF_ID 
															AND T6.MODULE_CODE = T1.COI_PROJECT_TYPE_CODE AND T6.PERSON_ID = T1.KEY_PERSON_ID
...
IF AV_TYPE ='A' THEN
    ...
    IF AV_REVIEW_STATUS IS NOT NULL AND AV_REVIEW_STATUS <> '' THEN 
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' AND T6.REVIEW_STATUS_CODE IN (', AV_REVIEW_STATUS, ') ');
    END IF;
...
IF AV_UNLIMITED = TRUE THEN
    ...
    FROM coi_project_proposal_v T1 ', LS_JOIN_CONDITION, ' WHERE (T6.FCOI_TYPE_CODE <> 1 OR T6.FCOI_TYPE_CODE IS NULL) 
...
SET LS_DYN_SQL = CONCAT(
    ...
    'WITH DistinctProjects AS (
        ...
        WHERE (T6.FCOI_TYPE_CODE <> 1 OR T6.FCOI_TYPE_CODE IS NULL) ', LS_FILTER_CONDITION, ' 
...
SELECT DISTINCT 
    ...
    T10.BADGE_COLOR,
    T1.UPDATE_TIMESTAMP,
	T12.QUESTIONNAIRE_COMPLETION_STATUS,
    CASE FN_EVAL_DISCLOSURE_STATUS(T1.KEY_PERSON_ID, T1.KEY_PERSON_ROLE_CODE, T1.COI_PROJECT_TYPE_CODE, T1.EXTERNAL_SYSTEM_REF_ID)
        WHEN 1 THEN ''Disclosure Required''
        WHEN -1 THEN ''Disclosure Not Required''
        ELSE ''Not Defined''
	END AS DISCLOSURE_NEEDED_STATUS,
    COALESCE(CC.COMMENT_COUNT, 0) AS COMMENT_COUNT,
    CASE 
        WHEN T6.REVIEW_STATUS_CODE IN (2, 3, 4, 7, 8) THEN "Yes" 
        ELSE "No"
	...
	LEFT JOIN CommentsCount CC ON CC.MODULE_ITEM_KEY = T1.EXTERNAL_SYSTEM_REF_ID
		AND CC.MODULE_CODE = T1.COI_PROJECT_TYPE_CODE
	LEFT JOIN (SELECT T1.KEY_PERSON_ID, T1.EXTERNAL_SYSTEM_REF_ID,
		CASE 
			WHEN COUNT(T2.QNR_ANSWER_ID) > 0 THEN ''Completed''
			ELSE ''Incomplete''
		END AS QUESTIONNAIRE_COMPLETION_STATUS
		FROM coi_project_proposal_v T1 LEFT JOIN coi_project_proposal_qnr_ans_v T2 
		ON T1.KEY_PERSON_ID = T2.KEY_PERSON_ID AND T1.EXTERNAL_SYSTEM_REF_ID = T2.PROPOSAL_NUMBER
        GROUP BY T1.KEY_PERSON_ID, T1.EXTERNAL_SYSTEM_REF_ID ) T12 ON T12.KEY_PERSON_ID = T1.KEY_PERSON_ID AND T12.EXTERNAL_SYSTEM_REF_ID = T1.EXTERNAL_SYSTEM_REF_ID', LS_JOIN_CONDITION, '
	WHERE T1.EXTERNAL_SYSTEM_REF_ID IN (SELECT PROJECT_ID FROM DistinctProjects)
	AND (T6.FCOI_TYPE_CODE <> 1 OR T6.FCOI_TYPE_CODE IS NULL) 
	', LS_FILTER_CONDITION, ' ORDER BY T1.UPDATE_TIMESTAMP DESC ;');
