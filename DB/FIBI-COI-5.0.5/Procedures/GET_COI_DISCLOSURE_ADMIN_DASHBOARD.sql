...
SET JOIN_CONDITION =  CONCAT(JOIN_CONDITION,' LEFT JOIN (SELECT IFNULL(COUNT(DISTINCT T2.PERSON_ENTITY_ID),0) AS NO_OF_SFI,T1.DISCLOSURE_ID
...
END IF;
	...
	SET SELECTED_FIELD_LIST = CONCAT(
									...
									CASE WHEN T1.FCOI_TYPE_CODE = 2 THEN
									    COALESCE(T14.PROPOSAL_TITLE, T21.IP_TITLE, T15.AWARD_TITLE) ELSE NULL END AS PROJECT_TITLE,
									CASE WHEN T1.FCOI_TYPE_CODE = 2 THEN
									    COALESCE(T14.PROPOSAL_ID, T21.IP_NUMBER, T15.AWARD_NUMBER) ELSE NULL END AS PROJECT_NUMBER,
									T22.BADGE_COLOR,
									T22.PROJECT_ICON,
	...
	SET JOIN_CONDITION = CONCAT(
								...
								LEFT JOIN (
									SELECT T1.DISCLOSURE_ID, GROUP_CONCAT(DISTINCT(T4.ENTITY_NAME) SEPARATOR ", ") AS ENTITY_NAMES,
								...
								LEFT JOIN (
										SELECT EXTERNAL_SYSTEM_REF_ID AS PROPOSAL_ID, TITLE AS PROPOSAL_TITLE, PROPOSAL_NUMBER, COI_PROJECT_TYPE_CODE
										FROM COI_PROJECT_PROPOSAL_V
									) T14 ON T14.PROPOSAL_NUMBER = T50.MODULE_ITEM_KEY AND T14.COI_PROJECT_TYPE_CODE = T1.COI_PROJECT_TYPE_CODE
								LEFT JOIN (
										SELECT AWARD_NUMBER , EXTERNAL_SYSTEM_REF_ID, TITLE AS AWARD_TITLE, COI_PROJECT_TYPE_CODE
										FROM COI_PROJECT_AWARD_V
									) T15 ON T15.AWARD_NUMBER = T50.MODULE_ITEM_KEY AND T15.COI_PROJECT_TYPE_CODE = T1.COI_PROJECT_TYPE_CODE
								LEFT JOIN (
										SELECT PROPOSAL_NUMBER AS IP_NUMBER , EXTERNAL_SYSTEM_REF_ID, TITLE AS IP_TITLE, COI_PROJECT_TYPE_CODE
										FROM COI_PROJECT_INSTITUTE_PROPOSAL_V
									) T21 ON T21.IP_NUMBER = T50.MODULE_ITEM_KEY AND T21.COI_PROJECT_TYPE_CODE = T1.COI_PROJECT_TYPE_CODE
								LEFT JOIN (
								...
								LEFT JOIN COI_PROJECT_TYPE T22 ON T22.COI_PROJECT_TYPE_CODE = T1.COI_PROJECT_TYPE_CODE ', JOIN_CONDITION
							);

        SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, ' FROM(SELECT DISTINCT ',SELECTED_FIELD_LIST,'
										FROM COI_DISCLOSURE T1
										INNER JOIN COI_DISCL_PROJECTS T50 ON T50.DISCLOSURE_ID = T1.DISCLOSURE_ID',
