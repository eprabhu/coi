...
CREATE VIEW `coi_project_award_v` AS
    SELECT
        ...
        `coi_int_stage_award`.`PROJECT_NUMBER` AS `EXTERNAL_SYSTEM_REF_ID`,
        ...
        `coi_int_stage_award`.`PROJECT_TYPE_CODE` AS `PROPOSAL_TYPE_CODE`,
    ...
    ORDER BY `coi_int_stage_award`.`SRC_SYS_UPDATE_TIMESTAMP` DESC
//
