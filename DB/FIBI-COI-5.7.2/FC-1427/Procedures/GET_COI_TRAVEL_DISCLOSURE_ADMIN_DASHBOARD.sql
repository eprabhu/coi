...
IF AV_TYPE ='A' THEN
				...
				SET LS_FILTER_CONDITION = CONCAT(
					LS_FILTER_CONDITION,
					' T.TRAVEL_START_DATE BETWEEN ''', AV_TRAVEL_START_DATE, '''',
					' AND ''', AV_TRAVEL_END_DATE, '''',
					' AND T.TRAVEL_END_DATE BETWEEN ''', AV_TRAVEL_START_DATE, '''',
					' AND ''', AV_TRAVEL_END_DATE, ''' AND '
				);

				ELSEIF AV_TRAVEL_START_DATE IS NOT NULL THEN
					SET LS_FILTER_CONDITION = CONCAT(
						LS_FILTER_CONDITION,
						' T.TRAVEL_START_DATE >= ''', AV_TRAVEL_START_DATE, ''' AND '
					);

				ELSEIF AV_TRAVEL_END_DATE IS NOT NULL THEN
					SET LS_FILTER_CONDITION = CONCAT(
						LS_FILTER_CONDITION,
						' T.TRAVEL_END_DATE <= ''', AV_TRAVEL_END_DATE, ''' AND '
					);
				END IF;
...
END IF;
	...
	SET JOIN_CONDITION = CONCAT(' LEFT JOIN ENTITY T2 ON T2.ENTITY_ID=T1.ENTITY_ID
							...
							LEFT JOIN (SELECT TD.TRAVEL_DISCLOSURE_ID, GROUP_CONCAT(DISTINCT c.COUNTRY_NAME ORDER BY c.COUNTRY_NAME ASC SEPARATOR '', '') AS TRAVEL_DESTINATIONS FROM COI_TRAVEL_DESTINATIONS TD LEFT JOIN COUNTRY C ON C.COUNTRY_CODE = TD.COUNTRY_CODE GROUP BY TD.TRAVEL_DISCLOSURE_ID) T50 ON T50.TRAVEL_DISCLOSURE_ID = T1.TRAVEL_DISCLOSURE_ID
