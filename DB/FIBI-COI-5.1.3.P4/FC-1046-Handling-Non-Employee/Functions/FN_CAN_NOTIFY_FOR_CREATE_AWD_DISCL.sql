...
BEGIN
    ...
    SET MODULE_PREFIX = SUBSTRING_INDEX(AV_MODULE_ITEM_KEY, '-', 1);
		...
		WHERE T2.KEY_PERSON_ID = AV_PERSON_ID AND T2.ATTRIBUTE_1_VALUE = 'Y' AND T1.PROJECT_NUMBER = AV_MODULE_ITEM_KEY  AND T1.PROJECT_STATUS_CODE IN (1, 6, 3);
		...
		FROM COI_INT_STAGE_AWARD T1
		INNER JOIN COI_INT_STAGE_AWARD_PERSON T2 ON T2.PROJECT_NUMBER = T1.PROJECT_NUMBER
		WHERE T2.KEY_PERSON_ID = AV_PERSON_ID AND T1.PROJECT_NUMBER = AV_MODULE_ITEM_KEY  AND T1.PROJECT_STATUS_CODE IN (1, 6, 3);
