...
FN_QRY_REVEWR_DAS_OVRVEW_TRAVEL

Purpose:
 - Build and return the complete dynamic SQL (TEXT) used by reviewer dashboard flows for the Travel module (module code 24).
 - The returned TEXT is ready for the caller to PREPARE/EXECUTE and implements CTEs, access filtering, joins, grouping, pagination and selected field lists depending on request flags.

Signature:
 - FUNCTION FN_QRY_REVEWR_DAS_OVRVEW_TRAVEL(AV_PERSON_ID VARCHAR(40), JSON_INPUT JSON) RETURNS TEXT

Inputs (via parameters / JSON_INPUT keys):
 - AV_PERSON_ID       : reviewer person identifier (embedded into returned SQL).
 - JSON_INPUT keys used:
     - FETCH_TYPE      : 'HEADER' (summary counts) or other (detail/count).
     - IS_COUNT        : boolean — when TRUE return SQL for a single TOTAL_COUNT.
     - LIMIT           : integer page size.
     - PAGED           : integer page index.
     - IS_UNLIMITED    : boolean — when TRUE disable LIMIT/OFFSET.
     - SORT_TYPE       : order expression (optional).
     - Additional filter keys forwarded to FN_QRY_RVW_DAS_OVRVW_TRAVEL_FIL_COND(AV_PERSON_ID, JSON_INPUT).

High-level behavior:
 - Extract JSON_INPUT fields into local variables and build dynamic TEXT pieces.
 - Build `ACCESS_TMP` CTE to compute accessible unit numbers
 - Compose LS_FILTER_CONDITION combining ACCESS_TMP membership, HOME/AFFILIATED unit checks, admin/assignee checks and the output of helper filter function.
 - For FETCH_TYPE = 'HEADER':
     - Compose selected summary fields (REVIEW_PENDING_COUNT, APPROVED_COUNT, etc.) and suppress pagination/sorting.
 - Else if IS_COUNT = TRUE:
     - Compose SELECT COUNT(DISTINCT T1.TRAVEL_DISCLOSURE_ID) AS TOTAL_COUNT (no pagination).
 - Else (detail mode):
     - Compose extensive SELECT field list (disclosure ids, unit/display names, review status, traveler info, custom fields, dates, flags, comment counts, aggregated relationship and destination data, etc.).
     - Append many LEFT JOIN fragments to enrich rows (ENTITY, UNIT, PERSON, custom element answers, funding types, destination aggregates, comment counts via FN_DISCLOSURE_TOTAL_COMMENT_COUNT, and helper field selectors).
     - Apply GROUP BY and aggregate expressions where needed.
     - Compute LIMIT/OFFSET when IS_UNLIMITED is not TRUE and apply ORDER BY using SORT_TYPE or default UPDATE_TIMESTAMP DESC.
 - Concatenate CTEs + SELECT into LS_DYN_SQL and RETURN the TEXT.
...
SET AV_SORT_TYPE        = JSON_UNQUOTE(JSON_EXTRACT(JSON_INPUT,'$.SORT_TYPE'));
    ...
    IF AV_FETCH_TYPE = 'HEADER' THEN
        SET SELECTED_FIELD_LIST = CONCAT(
            ' COUNT(DISTINCT CASE WHEN T1.REVIEW_STATUS_CODE IN (2,3) THEN T1.TRAVEL_DISCLOSURE_ID END) AS REVIEW_PENDING_COUNT,
              ...
              COUNT(DISTINCT CASE WHEN T1.REVIEW_STATUS_CODE IN (1,4,5) THEN T1.TRAVEL_DISCLOSURE_ID END) AS INPROGRESS_COUNT'
        );
        SET AV_SORT_TYPE = '';
    ...
    SET LS_DYN_SQL = CONCAT(LS_DYN_CTE_SQL, ' SELECT * FROM (SELECT ', SELECTED_FIELD_LIST, '
        FROM COI_TRAVEL_DISCLOSURE T1 ', JOIN_CONDITION, ' WHERE ', LS_FILTER_CONDITION, LS_GROUP_CONDITION, AV_SORT_TYPE, LS_OFFSET_CONDITION, ') T ');
