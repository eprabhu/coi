...
REVIEWER_DEPARTMENT_DASH_OVERVIEW

Purpose:
 - Produce department-level dashboard rows for reviewer widgets (FCOI module 8, OPA module 23, Travel module 24).
 - Return either a paged list of units with JSON count details per selected widget or a single total count row.

Signature:
 - PROCEDURE REVIEWER_DEPARTMENT_DASH_OVERVIEW(AV_PERSON_ID VARCHAR(40), JSON_INPUT JSON)

Inputs (from JSON_INPUT):
 - MODULE_CODE      : Optional module selector (string/number).
 - FETCH_TYPE       : Optional fetch mode (e.g., paging modes — handled via flags).
 - HOME_UNIT        : Optional unit name filter (partial match).
 - LIMIT, PAGED     : Pagination controls (LIMIT and page index).
 - UNLIMITED        : Boolean; when TRUE disables LIMIT/OFFSET.
 - IS_COUNT         : Boolean; when TRUE returns only total count.
 - Other flags used to control widget selection/state.

High level behavior:
 1. Convert JSON_INPUT to text and to lowercase for simple validation.
 2. Run two regex checks on the raw JSON text to block obvious SQL comment tokens and a list of dangerous SQL keywords.
    - Note: this is a brittle check operating on the raw JSON string (may yield false positives).
 3. Extract JSON values using JSON_EXTRACT/JSON_UNQUOTE into local variables.
 4. If HOME_UNIT provided, convert to wildcard pattern and append to WHERE filter.
 5. Ensure default widget preferences exist for the person (widgets 8001, 8002, 8003). If none exist, insert defaults and COMMIT.
 6. Build a recursive CTE base `UNIT_ACCESS_TMP` and then append module-specific CTE fragments conditionally:
    - For each active/selected widget (FCOI/OPA/TRAVEL) append:
      - RIGHT access CTE (e.g., FCOI_RIGHT_ACCESS_TMP) that uses PERSON_ROLES, ROLE_RIGHTS, RIGHTS and UNIT_WITH_CHILDREN to determine accessible units.
      - ACCESS CTE (e.g., FCOI_ACCESS_TMP / OPA_ACCESS_TMP / TRAVEL_ACCESS_TMP) that selects module items filtered by rights, assignee/admin checks, and active workflows.
      - JOIN clauses and JSON_OBJECT fragments are collected into `LS_JOIN_CONDITION`, `SELECTED_FIELD_LIST` and `LS_UNIT_FILTER_CONDITION`.
 7. After assembling CTEs and selected fields:
    - If IS_COUNT = TRUE then select COUNT(DISTINCT U.UNIT_NUMBER) AS TOTAL_COUNT.
    - Else return U.UNIT_NUMBER, U.UNIT_NAME, U.DISPLAY_NAME and JSON_ARRAY(...) AS COUNT_DETAILS, grouping by unit.
    - Apply HOME_UNIT filter, unit-access conditions, grouping and optionally LIMIT/OFFSET for pagination.
...
*/
    ...
    DECLARE LS_UNIT_FILTER_CONDITION        LONGTEXT DEFAULT '';
    DECLARE AV_INCLUDE_CHILD_UNITS          BOOLEAN;
    DECLARE LS_ORDER_BY_CONDITION           VARCHAR(600) DEFAULT '';
    DECLARE LS_PERSON_FILTER_CONDITION      VARCHAR(600) DEFAULT '';
    ...
    SET LS_PERSON_ID         = JSON_EXTRACT(JSON_INPUT,'$.PERSON');
    SET AV_INCLUDE_CHILD_UNITS  = JSON_EXTRACT(JSON_INPUT, '$.INCLUDE_CHILD_UNITS');

    IF AV_HOME_UNIT IS NOT NULL AND AV_HOME_UNIT <> '' AND (AV_INCLUDE_CHILD_UNITS IS NULL OR AV_INCLUDE_CHILD_UNITS = FALSE)THEN
    ...
    ELSEIF AV_INCLUDE_CHILD_UNITS = TRUE THEN
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' AND U.UNIT_NUMBER IN (SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN WHERE UNIT_NUMBER =''',AV_HOME_UNIT,''') ');
    END IF;

    IF LS_PERSON_ID IS NOT NULL AND LS_PERSON_ID <> '' THEN
        SET LS_PERSON_FILTER_CONDITION = CONCAT(LS_PERSON_FILTER_CONDITION, ' T1.PERSON_ID = ', LS_PERSON_ID, ' AND ');
    END IF;
    ...
    SET LS_DYN_CTE_SQL = CONCAT('
        WITH UNIT_ACCESS_TMP AS (SELECT distinct t1.UNIT_NUMBER, t1.UNIT_NAME, t1.PARENT_UNIT_NUMBER, t1.DISPLAY_NAME, t1.IS_ACTIVE,1 AS LVL
             FROM UNIT t1
             INNER JOIN unit_with_children t2 ON t2.UNIT_NUMBER = t1.UNIT_NUMBER
             WHERE t1.PARENT_UNIT_NUMBER is null AND EXISTS(SELECT 1 FROM PERSON WHERE STATUS = ''A'' AND HOME_UNIT = t2.CHILD_UNIT_NUMBER)
             UNION ALL
             SELECT distinct t1.UNIT_NUMBER,t1.UNIT_NAME,t1.PARENT_UNIT_NUMBER, t1.DISPLAY_NAME, t1.IS_ACTIVE,FN_GET_UNIT_HIERARCHY_LEVEL(t1.UNIT_NUMBER) AS LVL
             FROM UNIT t1
             INNER JOIN unit_with_children t2 ON t2.UNIT_NUMBER = t1.UNIT_NUMBER
             WHERE t1.PARENT_UNIT_NUMBER is not null AND EXISTS(SELECT 1 FROM PERSON WHERE STATUS = ''A'' AND HOME_UNIT = t2.CHILD_UNIT_NUMBER)
    ...
    IF LI_IS_WIDGET_ACTIVE = TRUE AND (LI_IS_WIDGET_SELECTED = TRUE OR LI_HAS_USER_PREFERENCE = FALSE) THEN
        ...
        SET LS_UNIT_FILTER_CONDITION = CONCAT(LS_UNIT_FILTER_CONDITION, '
            U.UNIT_NUMBER IN (SELECT HOME_UNIT FROM FCOI_ACCESS_TMP)
        ');

        IF LS_PERSON_FILTER_CONDITION IS NULl OR LS_PERSON_FILTER_CONDITION = '' THEN
            SET LS_UNIT_FILTER_CONDITION = CONCAT(LS_UNIT_FILTER_CONDITION,'
                OR U.UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM FCOI_RIGHT_ACCESS_TMP)
        ...
        SET LS_DYN_CTE_SQL = CONCAT(LS_DYN_CTE_SQL,',
            ...
            FCOI_ACCESS_TMP AS ( SELECT T1.* FROM COI_DISCLOSURE T1
                ...
                WHERE ', LS_PERSON_FILTER_CONDITION, ' (T1.HOME_UNIT IN (SELECT UNIT_NUMBER FROM FCOI_RIGHT_ACCESS_TMP)
    ...
    IF LI_IS_WIDGET_ACTIVE = TRUE AND (LI_IS_WIDGET_SELECTED = TRUE OR LI_HAS_USER_PREFERENCE = FALSE) THEN
        ...
        SET LS_UNIT_FILTER_CONDITION = CONCAT(LS_UNIT_FILTER_CONDITION, '
            U.UNIT_NUMBER IN (SELECT HOME_UNIT FROM OPA_ACCESS_TMP)
        ');
        IF LS_PERSON_FILTER_CONDITION IS NULl OR LS_PERSON_FILTER_CONDITION = '' THEN
            SET LS_UNIT_FILTER_CONDITION = CONCAT(LS_UNIT_FILTER_CONDITION,'
                OR U.UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM OPA_RIGHT_ACCESS_TMP)
            ');
        END IF;
        ...
        SET LS_DYN_CTE_SQL = CONCAT(LS_DYN_CTE_SQL,',
            ...
            OPA_ACCESS_TMP AS ( SELECT T1.* FROM OPA_DISCLOSURE T1
                ...
                WHERE ', LS_PERSON_FILTER_CONDITION, '(T1.VERSION_STATUS != ''ARCHIVE'' AND T1.HOME_UNIT IN (SELECT UNIT_NUMBER FROM OPA_RIGHT_ACCESS_TMP)
                    OR (T2.ASSIGNEE_PERSON_ID = ''', AV_PERSON_ID, ''' AND T2.REVIEW_STATUS_TYPE_CODE IN (1,2))
                    OR T1.ADMIN_PERSON_ID = ''', AV_PERSON_ID ,'''
                    OR exists  (SELECT 1 FROM WORKFLOW WF
                        INNER JOIN WORKFLOW_DETAIL WFD ON WF.WORKFLOW_ID = WFD.WORKFLOW_ID
    ...
    IF LI_IS_WIDGET_ACTIVE = TRUE AND (LI_IS_WIDGET_SELECTED = TRUE OR LI_HAS_USER_PREFERENCE = FALSE) THEN
        ...
        SET LS_UNIT_FILTER_CONDITION = CONCAT(LS_UNIT_FILTER_CONDITION,'
            U.UNIT_NUMBER IN (SELECT HOME_UNIT FROM TRAVEL_ACCESS_TMP)
        ');
        IF LS_PERSON_FILTER_CONDITION IS NULl OR LS_PERSON_FILTER_CONDITION = '' THEN
            SET LS_UNIT_FILTER_CONDITION = CONCAT(LS_UNIT_FILTER_CONDITION,'
                OR U.UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM TRAVEL_RIGHT_ACCESS_TMP)
            ');
        END IF;
        ...
        SET LS_DYN_CTE_SQL = CONCAT(LS_DYN_CTE_SQL,',
            ...
            TRAVEL_ACCESS_TMP AS (SELECT T1.* FROM COI_TRAVEL_DISCLOSURE T1
                WHERE ', LS_PERSON_FILTER_CONDITION, ' (T1.HOME_UNIT IN (SELECT UNIT_NUMBER FROM TRAVEL_RIGHT_ACCESS_TMP)
    ...
    IF SELECTED_FIELD_LIST IS NOT NULL AND SELECTED_FIELD_LIST <> '' THEN
        IF AV_IS_COUNT = TRUE THEN
            ...
            SET LS_ORDER_BY_CONDITION = ' ';
        ELSE
            IF RIGHT(SELECTED_FIELD_LIST, 2) = ', ' THEN
                SET SELECTED_FIELD_LIST = LEFT(SELECTED_FIELD_LIST, LENGTH(SELECTED_FIELD_LIST) - 2);
            END IF;
            SET SELECTED_FIELD_LIST = CONCAT(' U.UNIT_NUMBER, U.UNIT_NAME, U.DISPLAY_NAME,U.PARENT_UNIT_NUMBER, JSON_ARRAY(', SELECTED_FIELD_LIST, ' ) AS COUNT_DETAILS ');
            SET LS_GROUP_CONDITION = ' GROUP BY U.UNIT_NUMBER, U.UNIT_NAME ';
            SET LS_ORDER_BY_CONDITION = ' ORDER BY U.UNIT_NAME ASC ';
        END IF;

        IF LS_PERSON_ID IS NOT NULL AND LS_PERSON_ID <> '' THEN
            SET LS_JOIN_CONDITION = CONCAT(LS_JOIN_CONDITION, ' LEFT JOIN PERSON P on U.UNIT_NUMBER = P.HOME_UNIT ');
            SET LS_UNIT_FILTER_CONDITION = CONCAT(' (', LS_UNIT_FILTER_CONDITION, ' OR P.PERSON_ID = ', LS_PERSON_ID, ') ');
        END IF;

        SET LS_DYN_SQL = CONCAT(LS_DYN_CTE_SQL, ' SELECT ', SELECTED_FIELD_LIST,
            ' FROM UNIT_ACCESS_TMP U ', LS_JOIN_CONDITION,
            ' WHERE (', LS_UNIT_FILTER_CONDITION, ')', LS_FILTER_CONDITION, LS_GROUP_CONDITION, LS_ORDER_BY_CONDITION, LS_OFFSET_CONDITION);
