DELIMITER //
CREATE PROCEDURE `GET_COI_DISCLOSURE_REVIEWER_DASHBOARD_COUNT`(
...
DECLARE LS_REVIEWER_GROUP_CONDITION LONGTEXT;
...
IF AV_TAB_TYPE = 'NEW_SUBMISSIONS' THEN
		...
		SET TAB_QUERY = CONCAT(' AND T1.REVIEW_STATUS_CODE = ''3'' AND T17.REVIEW_STATUS_TYPE_CODE = ''1'' AND T1.VERSION_STATUS = ''PENDING'' ');

ELSEIF AV_TAB_TYPE = 'MY_REVIEWS' THEN
       
        SET TAB_QUERY = CONCAT(' AND T1.REVIEW_STATUS_CODE = ''3'' AND T17.REVIEW_STATUS_TYPE_CODE = ''3'' AND T1.VERSION_STATUS = ''PENDING'' ');

ELSEIF AV_TAB_TYPE = 'REVIEW_HISTORY' THEN
        
        SET TAB_QUERY = CONCAT(' AND T17.REVIEW_STATUS_TYPE_CODE = ''4''');
...
END IF;
        ...
        SET LS_DYN_SQL = CONCAT('SELECT  DISTINCT COUNT(*) FROM(SELECT DISTINCT
                                        ...
                                        INNER JOIN UNIT T22 ON T22.UNIT_NUMBER = T1.HOME_UNIT
										INNER JOIN COI_DISCLOSURE_FCOI_TYPE T8 ON T8.FCOI_TYPE_CODE = T1.FCOI_TYPE_CODE 
                                        ...
                                        LEFT JOIN (SELECT DISCLOSURE_NUMBER, VERSION_NUMBER, UPDATE_TIMESTAMP FROM COI_DISCLOSURE 
													WHERE (DISCLOSURE_NUMBER, VERSION_NUMBER) IN 
															(SELECT S1.DISCLOSURE_NUMBER, MAX(S1.VERSION_NUMBER) FROM COI_DISCLOSURE S1
															 WHERE S1.VERSION_STATUS = ''ACTIVE''
										...
										JOIN_CONDITION, ' WHERE T1.FCOI_TYPE_CODE IN (''1'',''2'',''3'',''4'') AND T17.ASSIGNEE_PERSON_ID = ''',AV_PERSON_ID,'''', TAB_QUERY,
