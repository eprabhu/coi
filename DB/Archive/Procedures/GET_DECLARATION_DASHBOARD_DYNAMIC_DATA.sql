...
IF AV_DASHBOARD_TYPE = 'A' THEN
    ...
    ELSEIF AV_TAB_TYPE = 'ALL_REVIEWS' THEN
        IF AV_TYPE = 'A' THEN
            -- Advanced search applied WITH review status
            IF AV_REVIEW_STATUS IS NOT NULL AND AV_REVIEW_STATUS <> '' THEN
                SET LS_FILTER_CONDITION = CONCAT(
                    LS_FILTER_CONDITION,
                    ' AND (T.REVIEW_STATUS_CODE IN (''',
                    REPLACE(AV_REVIEW_STATUS, ',', ''','''),
                    ''') AND (T.ADMIN_PERSON_ID = ''', AV_LOGIN_PERSON_ID, '''
                        OR T.HOME_UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM ACCESS))) '
                );
            -- Advanced search applied WITHOUT review status
            ELSE
                SET LS_FILTER_CONDITION = CONCAT(
                    LS_FILTER_CONDITION,
                    ' AND (T.REVIEW_STATUS_CODE IN (3,6)
                        AND (T.ADMIN_PERSON_ID = ''', AV_LOGIN_PERSON_ID, '''
                             OR T.HOME_UNIT_NUMBER IN (SELECT UNIT_NUMBER FROM ACCESS))) '
                );
            END IF;
