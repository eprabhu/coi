...
SET LS_FILTER_CONDITION = CONCAT(
    LS_FILTER_CONDITION,
    ' (T.UNIT IN (SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP)
       OR EXISTS (
            SELECT 1
        ...
        OR T.ADMIN_PERSON_ID = ''', AV_PERSON_ID, '''
    ) AND '
);
...
IF AV_TYPE ='A' THEN
						...
						END IF;
							SET LS_FILTER_CONDITION = CONCAT(
								LS_FILTER_CONDITION,
								' (T.TRAVELLER_NAME LIKE ''', AV_DISCLOSURE_PERSON, 
				...
				IF AV_ENTITY IS NOT NULL AND AV_ENTITY <> '' THEN
					...
					END IF;
						SET LS_FILTER_CONDITION = CONCAT(
							LS_FILTER_CONDITION,
							' (T.TRAVEL_ENTITY_NAME LIKE ''', AV_ENTITY, 
							''' OR T.ENTITY_ID LIKE ''', AV_ENTITY, ''') AND '
						);
					ELSE
						SET LS_FILTER_CONDITION = CONCAT(
							LS_FILTER_CONDITION,
							' T.ENTITY_ID = ''', AV_ENTITY, ''' AND '
						);
					END IF;
				END IF;
				...
				IF AV_COUNTRY IS NOT NULL AND AV_COUNTRY <> '' THEN
								...
								END IF;
									SET LS_FILTER_CONDITION = CONCAT(
										LS_FILTER_CONDITION,
										' (T.TRAVEL_DESTINATIONS LIKE ''', AV_COUNTRY, 
										''' OR T.COUNTRY_CODES LIKE ''', AV_COUNTRY, ''') AND '
									);
								ELSE
									SET LS_FILTER_CONDITION = CONCAT(
										LS_FILTER_CONDITION,
										' FIND_IN_SET(''', AV_COUNTRY, ''', REPLACE(T.COUNTRY_CODES, '' '', '''')) AND '
									);
								END IF;
							END IF;
...
END IF;
	...
	IF AV_IS_COUNT = TRUE THEN
		SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, ' SELECT DISTINCT COUNT(*) FROM ');

	ELSE
		SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, ' SELECT DISTINCT * FROM ');
