...
SET LS_DYN_SQL = CONCAT('SELECT ', AV_COLUMN_NAMES, ' FROM
    ...
    COALESCE(t5.PI_NAME, t10.PI_NAME, t6.PI_NAME) AS PRINCIPAL_INVESTIGATOR
	...
	LEFT JOIN (SELECT EXTERNAL_SYSTEM_REF_ID AS PROPOSAL_ID,
		TITLE  AS PROPOSAL_TITLE, EXTERNAL_SYSTEM_REF_ID, PI_NAME FROM COI_PROJECT_PROPOSAL_V WHERE PERSON_STATUS = ''A'') t5 ON t5.PROPOSAL_ID = t9.MODULE_ITEM_KEY AND t9.MODULE_CODE = 3
	LEFT  JOIN (SELECT AWARD_NUMBER, EXTERNAL_SYSTEM_REF_ID,
		TITLE AS AWARD_TITLE, PI_NAME FROM COI_PROJECT_AWARD_V WHERE PERSON_STATUS = ''A'') t6  ON t6.AWARD_NUMBER = t9.MODULE_ITEM_KEY AND t9.MODULE_CODE = 1
	LEFT  JOIN (SELECT PROPOSAL_NUMBER AS IP_NUMBER , EXTERNAL_SYSTEM_REF_ID,
		TITLE AS IP_TITLE,PI_NAME FROM COI_PROJECT_INSTITUTE_PROPOSAL_V WHERE PERSON_STATUS = ''A'') t10 ON t10.IP_NUMBER = t9.MODULE_ITEM_KEY AND t9.MODULE_CODE = 2
