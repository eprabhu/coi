...
CREATE PROCEDURE `COI_DISCL_PROJ_ENTITY_INSERTION`(
	...
	LIMIT 1;
        ...
        LEFT JOIN (
            ...
            LEFT JOIN coi_int_stage_proposal IP ON IP.LINKED_AWARD_PROJECT_NUMBER = AV_MODULE_ITEM_KEY AND T.MODULE_CODE = 2
            LEFT JOIN coi_int_stage_dev_proposal PP ON PP.IP_NUMBER = AV_MODULE_ITEM_KEY AND T.MODULE_CODE = 3
            WHERE IP.PROJECT_NUMBER = T.MODULE_ITEM_KEY OR PP.PROPOSAL_NUMBER = T.MODULE_ITEM_KEY
        ) t4 on T1.COI_DISCL_PROJECTS_ID = t4.COI_DISCL_PROJECTS_ID
        WHERE T1.PERSON_ENTITY_NUMBER = LI_PERSON_ENTITY_NUMBER AND T1.ENTITY_ID = LI_ENTITY_ID
          AND ((T2.MODULE_CODE = AV_MODULE_CODE AND T2.MODULE_ITEM_KEY = AV_MODULE_ITEM_KEY)
              OR t4.COI_DISCL_PROJECTS_ID is not null
          )
