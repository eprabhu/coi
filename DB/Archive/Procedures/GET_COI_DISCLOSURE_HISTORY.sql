...
BEGIN
	...
	DECLARE IS_TRAVEL_ACTIVE VARCHAR(1) DEFAULT 'N';
	DECLARE IS_CONSULTING_ACTIVE VARCHAR(1) DEFAULT 'N';
	DECLARE LS_JOIN_CONDITION			LONGTEXT;
    DECLARE LS_ACCESS_CONDITION         TEXT;
    DECLARE LS_ACCESS_FCOI_CONDITION    TEXT;
    DECLARE LS_ACCESS_PROJECT_CONDITION TEXT;
	DECLARE LS_ACCESS_TRAVEL_CONDITION  TEXT;
	DECLARE LS_ACCESS_CONSULTING_CONDITION  TEXT;
	...
	SET LS_ACCESS_FCOI_CONDITION = CONCAT(' ACCESS_FCOI AS
										   ...
										   UNION
										   SELECT CHILD_UNIT_NUMBER AS UNIT_NUMBER 
	...
	SET LS_ACCESS_PROJECT_CONDITION = CONCAT(' ACCESS_PROJECT AS
										   (
										   SELECT UNIT_NUMBER
										   FROM PERSON_ROLES T1
										   INNER JOIN ROLE_RIGHTS T2 ON T1.ROLE_ID = T2.ROLE_ID
   ...
   SET LS_ACCESS_TRAVEL_CONDITION = CONCAT(' ACCESS_TRAVEL AS
											   (
											   SELECT UNIT_NUMBER
											   FROM PERSON_ROLES T1
											   INNER JOIN ROLE_RIGHTS T2 ON T1.ROLE_ID = T2.ROLE_ID
											   INNER JOIN RIGHTS T3 ON T2.RIGHT_ID = T3.RIGHT_ID
											   WHERE T1.DESCEND_FLAG = ''N'' AND T1.PERSON_ID = ''',AV_LOGIN_PERSON_ID,'''
											   AND RIGHT_NAME IN (''VIEW_TRAVEL_DISCLOSURE'', ''MANAGE_TRAVEL_DISCLOSURE'')
											   UNION
											   SELECT CHILD_UNIT_NUMBER AS UNIT_NUMBER 
											   FROM UNIT_WITH_CHILDREN
											   WHERE UNIT_NUMBER IN (
	...
	SET LS_ACCESS_CONSULTING_CONDITION = CONCAT(' ACCESS_CONSULTING AS
											   (
											   SELECT UNIT_NUMBER
											   FROM PERSON_ROLES T1
											   INNER JOIN ROLE_RIGHTS T2 ON T1.ROLE_ID = T2.ROLE_ID
        ...
        SELECT
            MAX(CASE WHEN DISCLOSURE_TYPE_CODE = '1' THEN IS_ACTIVE END),
            MAX(CASE WHEN DISCLOSURE_TYPE_CODE = '3' THEN IS_ACTIVE END),
            MAX(CASE WHEN DISCLOSURE_TYPE_CODE = '4' THEN IS_ACTIVE END)
            INTO IS_FINANCIAL_ACTIVE, IS_TRAVEL_ACTIVE, IS_CONSULTING_ACTIVE
            FROM coi_disclosure_type
            WHERE DISCLOSURE_TYPE_CODE IN ('1', '3', '4');
        ...
        IF AV_FILTER_TYPE = 'ALL' THEN
			...
			SET LS_ACCESS_CONDITION = CONCAT('WITH ', LS_ACCESS_FCOI_CONDITION, ', ',LS_ACCESS_PROJECT_CONDITION);
    		...
    		SET ALL_DISC_SELECTED_FIELD_LIST= CONCAT('T1.DISCLOSURE_ID, NULL AS TRAVEL_DISCLOSURE_ID, NULL AS CONSULTING_DISCLOSURE_ID, T1.UPDATE_TIMESTAMP, T1.VERSION_STATUS, T1.FCOI_TYPE_CODE,
											...
											OR EXISTS (SELECT 1 FROM COI_REVIEW CR WHERE CR.DISCLOSURE_ID = T1.DISCLOSURE_ID AND CR.ASSIGNEE_PERSON_ID = ''', AV_LOGIN_PERSON_ID,''')  
											OR WFR.APPROVER_PERSON_ID IS NOT NULL
											OR T1.HOME_UNIT IN (SELECT UNIT_NUMBER FROM ACCESS_FCOI)) AS IS_VIEW_ALLOWED ');
    		...
    		SET ALL_DISC_JOIN_CONDITION = CONCAT(' FROM COI_DISCLOSURE T1
										...
										SELECT DISTINCT T1.DISCLOSURE_ID, NULL AS TRAVEL_DISCLOSURE_ID, NULL AS CONSULTING_DISCLOSURE_ID, T1.UPDATE_TIMESTAMP, T1.VERSION_STATUS, T1.FCOI_TYPE_CODE,
											...
											OR EXISTS (SELECT 1 FROM COI_REVIEW CR WHERE CR.DISCLOSURE_ID = T1.DISCLOSURE_ID AND CR.ASSIGNEE_PERSON_ID = ''', AV_LOGIN_PERSON_ID,''')  
											OR WFR.APPROVER_PERSON_ID IS NOT NULL
											OR T1.HOME_UNIT IN (SELECT UNIT_NUMBER FROM ACCESS_PROJECT)) AS IS_VIEW_ALLOWED
        ...
        ELSE IF AV_FILTER_TYPE = 'FCOI' THEN
			...
			SET LS_ACCESS_CONDITION = CONCAT('WITH ', LS_ACCESS_FCOI_CONDITION);
        ...
        ELSE IF AV_FILTER_TYPE = 'PROJECT' THEN
			...
			SET LS_ACCESS_CONDITION = CONCAT('WITH ', LS_ACCESS_PROJECT_CONDITION);
    	...
    	IF AV_FILTER_TYPE = 'ALL' OR AV_FILTER_TYPE = 'TRAVEL' THEN
			IF LENGTH(LS_ACCESS_CONDITION) > 0 THEN
				SET LS_ACCESS_CONDITION = CONCAT(LS_ACCESS_CONDITION, ', ', LS_ACCESS_TRAVEL_CONDITION);
			ELSE
				SET LS_ACCESS_CONDITION = CONCAT('WITH ', LS_ACCESS_TRAVEL_CONDITION);
			END IF;
    		...
    		SET TRAVEL_SELECTED_FIELD_LIST= CONCAT('NULL AS DISCLOSURE_ID, T21.TRAVEL_DISCLOSURE_ID, NULL AS CONSULTING_DISCLOSURE_ID, T21.UPDATE_TIMESTAMP, T21.VERSION_STATUS, NULL AS  FCOI_TYPE_CODE,
                                            ...
                                            NULL AS CONFLICT_STATUS, NULL AS DISPOSITION_STATUS_CODE, NULL AS DISPOSITION_STATUS, T21.REVIEW_STATUS_CODE, T24.DESCRIPTION AS REVIEW_STATUS,
											...
											,NULL AS DOCUMENT_NUMBER,NULL AS PROJECT_ID,
                                            (T21.PERSON_ID = ''', AV_LOGIN_PERSON_ID,'''
											OR T21.HOME_UNIT IN (SELECT UNIT_NUMBER FROM ACCESS_TRAVEL)) AS IS_VIEW_ALLOWED ');
    		SET TRAVEL_DISC_JOIN_CONDITION = CONCAT(' FROM COI_TRAVEL_DISCLOSURE T21 INNER JOIN ENTITY T22 ON T22.ENTITY_ID = T21.ENTITY_ID
                                            ...
                                            LEFT JOIN COI_TRAVEL_STATUS T23 ON T23.TRAVEL_STATUS_CODE = T21.TRAVEL_STATUS_CODE 
                                            LEFT JOIN COI_TRAVEL_REVIEW_STATUS T24 on t24.REVIEW_STATUS_CODE = T21.REVIEW_STATUS_CODE WHERE T21.PERSON_ID = ''',AV_PERSON_ID,'''');
    	END IF;
    	IF AV_FILTER_TYPE = 'ALL' OR AV_FILTER_TYPE = 'CONSULTING' THEN
		
			IF LENGTH(LS_ACCESS_CONDITION) > 0 THEN
				SET LS_ACCESS_CONDITION = CONCAT(LS_ACCESS_CONDITION, ', ', LS_ACCESS_CONSULTING_CONDITION);
			ELSE
				SET LS_ACCESS_CONDITION = CONCAT('WITH ', LS_ACCESS_CONSULTING_CONDITION);
			END IF;
    		...
    		SET CONSULTING_SELECTED_FIELD_LIST= CONCAT('NULL AS DISCLOSURE_ID, NULL AS TRAVEL_DISCLOSURE_ID, T50.DISCLOSURE_ID AS CONSULTING_DISCLOSURE_ID,
                                            ...
                                            NULL AS PROJECT_TITLE, NULL AS PROJECT_NUMBER, NULL AS COI_PROJECT_TYPE_CODE,NULL AS PROJECT_TYPE,NULL AS BADGE_COLOR,NULL AS PROJECT_ICON
											,NULL AS DOCUMENT_NUMBER,NULL AS PROJECT_ID,
                                            (T50.PERSON_ID = ''', AV_LOGIN_PERSON_ID,'''
											OR T50.HOME_UNIT IN (SELECT UNIT_NUMBER FROM ACCESS_CONSULTING)) AS IS_VIEW_ALLOWED ');
    		SET CONSULTING_DISC_JOIN_CONDITION = CONCAT(' FROM CONSULTING_DISCLOSURE T50
    										LEFT JOIN ENTITY T54 on T54.ENTITY_ID = T50.ENTITY_ID
        ...
        IF AV_FILTER_TYPE = 'ALL' THEN
    		SET @dynamicQueryBody  = '';
            IF IS_FINANCIAL_ACTIVE = 'Y' THEN
                SET @dynamicQueryBody  = CONCAT(@dynamicQueryBody , 
                    '(SELECT DISTINCT ', ALL_DISC_SELECTED_FIELD_LIST, ALL_DISC_JOIN_CONDITION, DISC_TAB_QUERY, ')');
            END IF;

			IF IS_TRAVEL_ACTIVE = 'Y' THEN
				IF LENGTH(@dynamicQueryBody ) > 0 THEN
					SET @dynamicQueryBody  = CONCAT(@dynamicQueryBody , ' UNION ALL ');
				END IF;
				SET @dynamicQueryBody  = CONCAT(@dynamicQueryBody , 
					'(SELECT DISTINCT ', TRAVEL_SELECTED_FIELD_LIST, TRAVEL_DISC_JOIN_CONDITION, TRAVEL_TAB_QUERY, ')');
			END IF;

			IF IS_CONSULTING_ACTIVE = 'Y' THEN
				IF LENGTH(@dynamicQueryBody ) > 0 THEN
					SET @dynamicQueryBody  = CONCAT(@dynamicQueryBody , ' UNION ALL ');
				END IF;
				SET @dynamicQueryBody  = CONCAT(@dynamicQueryBody , 
					'(SELECT DISTINCT ', CONSULTING_SELECTED_FIELD_LIST, CONSULTING_DISC_JOIN_CONDITION, CONSULTING_TAB_QUERY, ')');
			END IF;
            
            IF LENGTH(@dynamicQueryBody) > 0 THEN
				SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, ' (', LS_ACCESS_CONDITION, @dynamicQueryBody , ') T ORDER BY T.UPDATE_TIMESTAMP DESC');
			ELSE
				IF AV_IS_COUNT THEN
					SET LS_DYN_SQL = 'SELECT 0';
				ELSE
                    SET LS_DYN_SQL = 'SELECT NULL';
				END IF;
			END IF;

        ELSEIF AV_FILTER_TYPE = 'TRAVEL' THEN
    		SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, '(', LS_ACCESS_CONDITION,'SELECT DISTINCT ',TRAVEL_SELECTED_FIELD_LIST, TRAVEL_DISC_JOIN_CONDITION, TRAVEL_TAB_QUERY, ')T ORDER BY T.UPDATE_TIMESTAMP DESC');

    	ELSEIF AV_FILTER_TYPE = 'CONSULTING' THEN
    		SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, '(', LS_ACCESS_CONDITION,'SELECT DISTINCT ',CONSULTING_SELECTED_FIELD_LIST, CONSULTING_DISC_JOIN_CONDITION, CONSULTING_TAB_QUERY, ')T ORDER BY T.UPDATE_TIMESTAMP DESC');

        ELSE
    		SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, '(', LS_ACCESS_CONDITION,'SELECT DISTINCT ',DISC_SELECTED_FIELD_LIST, DISC_JOIN_CONDITION, DISC_TAB_QUERY, ') T ORDER BY T.UPDATE_TIMESTAMP DESC');
        END IF;
