	...
	IF LS_COUNT > 0 THEN 
		...
		ELSE IF AV_MODULE_CODE = 2 THEN 
			...
			SELECT DISTINCT 
                ...
                CASE 
                WHEN (
                    (T2.KEY_PERSON_DISCL_REQUIREMENT = 'ALL' OR 
                    (T2.KEY_PERSON_DISCL_REQUIREMENT = 'PI-COI' AND T7.KEY_PERSON_ROLE_CODE IN (1,3)))
                    OR
                    (T3.KEY_PERSON_DISCL_REQUIREMENT = 'ALL' OR 
                    (T3.KEY_PERSON_DISCL_REQUIREMENT = 'PI-COI' AND T7.KEY_PERSON_ROLE_CODE IN (1,3)))
                    OR
                    (T6.ATTRIBUTE_1_VALUE = 'PC' AND T7.KEY_PERSON_ROLE_CODE IN (1,3))
                    OR
                    (T6.ATTRIBUTE_1_VALUE = 'PCK')
                ) THEN 'Y'
                ELSE 'N'
            END AS DISCLOSURE_REQUIRED_FLAG
			...
			LEFT JOIN ROLODEX T18 ON T18.ROLODEX_ID = T7.KEY_PERSON_ID AND T7.ATTRIBUTE_1_VALUE = 'Y' -- NON_EMPLOYEE_FLAG
			LEFT JOIN UNIT T19 ON T19.UNIT_NUMBER = T18.OWNED_BY_UNIT
            LEFT JOIN SPONSOR_DISCLOSURE_REQUIREMENTS T2 ON T2.SPONSOR_CODE = T6.SPONSOR_CODE
            LEFT JOIN SPONSOR_DISCLOSURE_REQUIREMENTS T3 ON T3.SPONSOR_CODE = T6.PRIME_SPONSOR_CODE
            WHERE T6.PROJECT_NUMBER = AV_PROJECT_NUMBER ORDER BY T15.SORT_ID;
		ELSE IF AV_MODULE_CODE = 3 THEN 
			...
			SELECT DISTINCT 
				...
				T7.ATTRIBUTE_1_VALUE AS NON_EMPLOYEE_FLAG,
                CASE 
                WHEN T7.DISCLOSURE_REQUIRED_FLAG = 'REQUIRED' THEN 'Y'
                WHEN T7.DISCLOSURE_REQUIRED_FLAG IS NULL THEN NULL
                ELSE 'N'
                END AS DISCLOSURE_REQUIRED_FLAG
