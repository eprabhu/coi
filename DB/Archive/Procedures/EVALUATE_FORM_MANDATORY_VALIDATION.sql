...
LIMIT 1;
			...
			-- Precompute number of options
				...
				SELECT CASE
					...
					WHEN (SELECT ANSWER_TYPE FROM quest_question WHERE QUESTION_ID = LI_QUESTION_ID) = 'Table' THEN
						CASE 
							-- Check if no rows exist → incomplete
							WHEN NOT EXISTS (
								SELECT 1
							...
							-- Otherwise, check if any row incomplete
							WHEN EXISTS (
								SELECT 1
								FROM fb_quest_table_answer TA
								WHERE TA.QUESTION_ID = LI_QUESTION_ID
				...
				INTO LI_CNT;
                 IF LI_CNT = 0 THEN
			      IF LI_RULE_ID IS NOT NULL THEN
				   ...
				   -- CALL RULE_EVALUATION_OUT(AV_MODULE_CODE, AV_MODULE_CODE, AV_SUBMODULE_CODE, AV_MODULE_ITEM_KEY,LI_RULE_ID, '10000000001','10000000001', AV_SUB_MODULE_ITEM_KEY, @RESULT);
				-- IF @RESULT = 0 THEN
				 ITERATE QUEST_LOOP;
				 -- END IF;
				END IF;
						...
						WHERE
                            ...
                            END IF;
                        ELSE
                            -- Regular mandatory unanswered question
                            INSERT INTO RESULT_TABLE
                            VALUES ('VM', LS_Q_MANDATORY_MESSAGE, LI_FORM_BUILDER_SECT_COMP_ID, LI_QUESTION_ID);
                        END IF;
                    END IF;
