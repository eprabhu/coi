...
ELSEIF AV_ROLE_TYPE = 8068 THEN -- Cancelled Approvers
        IF AV_MODULE_CODE IN (8, 23) THEN
                ...
                AND T2.APPROVAL_STATUS IN ('C');
        END IF;

ELSEIF AV_ROLE_TYPE = 8070 THEN -- Role Type: Research Compliance Help
	IF AV_MODULE_CODE = 28 THEN
                SELECT DISTINCT PR.PERSON_ID AS PERSON_ID
		FROM PERSON_ROLES PR
		JOIN PERSON P ON P.PERSON_ID = PR.PERSON_ID
		WHERE PR.ROLE_ID = 1387 -- Role: Research Compliance Help
		AND P.STATUS = 'A';
	END IF;

ELSEIF AV_ROLE_TYPE = 8072 THEN -- Role Type: OPA Review Assigner
	IF AV_MODULE_CODE = 23 THEN
                SELECT DISTINCT T2.PERSON_ID AS PERSON_ID
		FROM OPA_REVIEW T1
		JOIN PERSON T2 ON T1.CREATE_USER = T2.USER_NAME
		WHERE T1.OPA_DISCLOSURE_ID = AV_MODULE_ITEM_ID
        AND T1.OPA_REVIEW_ID = AV_SUB_MODULE_ITEM_KEY;
	END IF;
