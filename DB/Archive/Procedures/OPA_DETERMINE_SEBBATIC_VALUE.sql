...
DECLARE LS_IS_FALL_SABATICAL VARCHAR(1) DEFAULT 'N';
    ...
    IF (LS_SABBATICAL_END_DATE IS NOT NULL AND LS_SABBATICAL_END_DATE BETWEEN DATE_SUB(CURDATE(), INTERVAL 12 MONTH) AND CURDATE()) THEN
        IF (MONTH(LS_SABBATICAL_END_DATE) < 9 AND
            ...
            SET LS_IS_SPRING_SABATICAL = 'Y';
   		ELSEIF (YEAR(LS_SABBATICAL_END_DATE) != YEAR(LS_SABBATICAL_BEGIN_DATE))THEN
            SET LS_IS_FALL_SABATICAL = 'Y';
            SET LS_IS_SPRING_SABATICAL = 'Y';
    ...
    IF (LS_SABBATICAL_BEGIN_DATE < CURDATE() AND LS_SABBATICAL_END_DATE > CURDATE() AND YEAR(LS_SABBATICAL_END_DATE) != YEAR(LS_SABBATICAL_BEGIN_DATE)) THEN
        SET LS_IS_FALL_SABATICAL = 'Y';
        SET LS_IS_SPRING_SABATICAL = 'Y';
    ELSEIF (LS_SABBATICAL_BEGIN_DATE < CURDATE() AND LS_SABBATICAL_END_DATE > CURDATE() AND YEAR(LS_SABBATICAL_END_DATE) = YEAR(LS_SABBATICAL_BEGIN_DATE)
            AND MONTH(LS_SABBATICAL_BEGIN_DATE) NOT BETWEEN 6 AND 8 AND MONTH(LS_SABBATICAL_END_DATE) NOT BETWEEN 6 AND 8) THEN
        IF (MONTH(LS_SABBATICAL_BEGIN_DATE) BETWEEN 1 AND 5 AND MONTH(LS_SABBATICAL_END_DATE) BETWEEN 9 AND 12) THEN
            SET LS_IS_FALL_SABATICAL = 'Y';
            SET LS_IS_SPRING_SABATICAL = 'Y';
        ELSEIF (MONTH(LS_SABBATICAL_BEGIN_DATE) BETWEEN 9 AND 12 AND MONTH(LS_SABBATICAL_END_DATE) BETWEEN 9 AND 12) THEN
            SET LS_IS_FALL_SABATICAL = 'Y';
        ELSEIF (MONTH(LS_SABBATICAL_BEGIN_DATE) BETWEEN 1 AND 5 AND MONTH(LS_SABBATICAL_END_DATE) BETWEEN 1 AND 5) THEN
            SET LS_IS_FALL_SABATICAL = 'Y';
        END IF;
    ELSEIF (LS_SABBATICAL_BEGIN_DATE < CURDATE() AND LS_SABBATICAL_END_DATE > CURDATE() AND YEAR(LS_SABBATICAL_END_DATE) = YEAR(LS_SABBATICAL_BEGIN_DATE)
         AND MONTH(LS_SABBATICAL_BEGIN_DATE)  BETWEEN 6 AND 8 AND MONTH(LS_SABBATICAL_END_DATE) BETWEEN 9 AND 12) THEN
         SET LS_IS_FALL_SABATICAL = 'Y';
    ELSEIF (LS_SABBATICAL_BEGIN_DATE < CURDATE() AND LS_SABBATICAL_END_DATE > CURDATE() AND YEAR(LS_SABBATICAL_END_DATE) = YEAR(LS_SABBATICAL_BEGIN_DATE)
         AND MONTH(LS_SABBATICAL_BEGIN_DATE)  BETWEEN 1 AND 5 AND MONTH(LS_SABBATICAL_END_DATE) BETWEEN 6 AND 8) THEN
         SET LS_IS_SPRING_SABATICAL = 'Y';
    END IF;
