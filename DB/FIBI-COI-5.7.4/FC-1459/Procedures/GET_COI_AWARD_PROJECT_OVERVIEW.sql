...
FROM (SELECT 
    ...
    T1.ACCOUNT_NUMBER,
    CASE 
   -- WHEN T1.DISCLOSURE_REQUIRED_FLAG = ''NOT_REQUIRED'' AND T1.DISCLOSURE_VALIDATION_FLAG = ''SELF'' THEN ''Not Required''
    ...
    END AS PERSON_SUBMISSION_STATUS ,
 	CASE
   --  WHEN T1.DISCLOSURE_REQUIRED_FLAG = ''NOT_REQUIRED'' AND T1.DISCLOSURE_VALIDATION_FLAG = ''SELF'' THEN ''N/A''
     WHEN T1.REVIEW_STATUS = ''COMPLETED'' THEN ''Completed''
...
(SELECT FN_GET_OVAL_AWD_PRJT_DISCL_STATUS(T1.EXTERNAL_SYSTEM_REF_ID)) AS PROJECT_SUBMISSION_STATUS,
    ...
    COUNT(CASE WHEN DAP.DISCLOSURE_STATUS != ''DISCLOSURE_COMPLETED'' OR DAP.DISCLOSURE_STATUS IS NULL THEN 1 END) AS Incomplete_count,
  ...
  COALESCE(PC.PERSON_COMMENT_COUNT, 0) AS PERSON_COMMENT_COUNT
	...
	WHERE PERSON_STATUS != ''I'' ',IFNULL(LS_FILTER_CONDITION,''), ' GROUP BY T1.EXTERNAL_SYSTEM_REF_ID , T1.KEY_PERSON_ID,T1.KEY_PERSON_ROLE_CODE )T1',ifnull(LS_ORDER_BY,''),' )T '
