...
IF AV_TAB_TYPE = 'CONSULTING_DISCLOSURES' THEN
		...
		SET LS_DYN_SQL = CONCAT(' SELECT  *  FROM (SELECT DISTINCT
							T50.DISCLOSURE_ID,
...
ELSEIF AV_TAB_TYPE = 'TRAVEL_DISCLOSURES' THEN
		...
		SET LS_DYN_SQL = CONCAT(' SELECT DISTINCT *  FROM(SELECT DISTINCT
							...
							LEFT JOIN(SELECT T21.TRAVEL_DISCLOSURE_ID, GROUP_CONCAT(DISTINCT T25.DESCRIPTION ORDER BY T25.TRAVELER_TYPE_CODE ASC) AS RELATIONSHIP_TYPES 
								FROM COI_TRAVEL_DISCLOSURE T21 
...
ELSE
        ...
        SET LS_DYN_SQL =CONCAT('SELECT DISTINCT *  FROM(SELECT DISTINCT
										...
										' )T ',LS_FILTER_CONDITION,' ',AV_SORT_TYPE);
END IF;
