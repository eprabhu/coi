...
BEGIN
    ...
    SEL_PERSONS_LOOP: LOOP
        ...
        IF UPPER(LS_DISCLOSURE_REQUIRED_FLAG) = 'REQUIRED' OR UPPER(LS_DISCLOSURE_REQUIRED_FLAG) = 'TO_BE_DETERMINED' THEN 
			...
			SELECT CD.REVIEW_STATUS_CODE INTO LS_REVIEW_STATUS  
			...
			-- If review status is NULL, check legacy disclosure details
			IF LS_REVIEW_STATUS IS NULL THEN
				SELECT CAST(CD.REVIEW_STATUS_CODE AS CHAR) INTO LS_REVIEW_STATUS
				FROM LEGACY_COI_DISC_DETAILS CDP
				INNER JOIN LEGACY_COI_DISCLOSURE CD ON CD.LEGACY_DISCLOSURE_ID = CDP.LEGACY_DISCLOSURE_ID
				WHERE CDP.MODULE_CODE = 3
				-- AND CD.EVENT_TYPE_CODE IN (1, 2)
				AND CDP.MODULE_ITEM_KEY = AV_PROPOSAL_NUMBER
				AND CD.PERSON_ID =  LS_KEY_PERSON_ID
				AND CD.DISCLOSURE_DISPOSITION_CODE = 1 -- Disposition code 1: Approved
            ...
            IF LS_REVIEW_STATUS = '1' OR LS_REVIEW_STATUS IS NULL THEN 
                RETURN -1;			
            END IF;
