...
BEGIN
    ...
    IF AV_FILTER_TYPE = 'ALL' OR AV_FILTER_TYPE = 'FCOI' OR AV_FILTER_TYPE = 'PROJECT' THEN
		...
		SET DISC_SELECTED_FIELD_LIST= CONCAT('T1.DISCLOSURE_ID, NULL AS TRAVEL_DISCLOSURE_ID,  T1.UPDATE_TIMESTAMP, T1.VERSION_STATUS, T1.FCOI_TYPE_CODE, 
                                        ...
                                        CASE T1.MODULE_CODE WHEN 3 THEN T7.TITLE WHEN 1 THEN T8.TITLE ELSE NULL END AS PROJECT_TITLE, 
                                        CASE T1.MODULE_CODE WHEN 3 THEN T7.EXTERNAL_SYSTEM_REF_ID WHEN 1 THEN T8.AWARD_NUMBER ELSE NULL END AS PROJECT_NUMBER');
                        
		SET DISC_JOIN_CONDITION = CONCAT(' FROM COI_DISCLOSURE T1 INNER JOIN COI_DISCLOSURE_FCOI_TYPE T2 ON T2.FCOI_TYPE_CODE = T1.FCOI_TYPE_CODE
                                    ...
                                    LEFT JOIN COI_PROJECT_PROPOSAL_V T7 ON (T7.EXTERNAL_SYSTEM_REF_ID = CAST(T1.MODULE_ITEM_KEY AS UNSIGNED) AND T1.MODULE_CODE = 3)
                                    LEFT JOIN COI_PROJECT_AWARD_V T8 ON (T8.EXTERNAL_SYSTEM_REF_ID = CAST(T1.MODULE_ITEM_KEY AS UNSIGNED) AND T1.MODULE_CODE = 1)
	...
	IF AV_FILTER_TYPE = 'ALL' OR AV_FILTER_TYPE = 'TRAVEL' THEN
		...
		SET TRAVEL_SELECTED_FIELD_LIST= CONCAT('NULL AS DISCLOSURE_ID, T21.TRAVEL_DISCLOSURE_ID, T21.UPDATE_TIMESTAMP, T21.VERSION_STATUS, NULL AS  FCOI_TYPE_CODE,
                                        ...
                                        T22.ENTITY_NAME, T21.DESTINATION_COUNTRY, T21.STATE, T21.DESTINATION_CITY, T21.PURPOSE_OF_THE_TRIP,
    ...
    IF AV_FILTER_TYPE = 'ALL' THEN
		SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, '(SELECT DISTINCT ',DISC_SELECTED_FIELD_LIST, DISC_JOIN_CONDITION, DISC_TAB_QUERY, ' UNION SELECT DISTINCT ',
							TRAVEL_SELECTED_FIELD_LIST, TRAVEL_DISC_JOIN_CONDITION, TRAVEL_TAB_QUERY, ' ) T ORDER BY UPDATE_TIMESTAMP DESC ');
