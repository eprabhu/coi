...
SET LS_DYN_SQL = CONCAT('SELECT 
                        ...
                        CASE WHEN T1.IS_TRAVEL = ''N'' AND T1.IS_FINANCIAL = ''Y'' THEN ''Financial Only''
							WHEN T1.IS_TRAVEL = ''Y'' AND T1.IS_FINANCIAL = ''N'' THEN ''Travel'' 
							ELSE ''Financial with travel details''
						...
						WHERE PERSON_ID =''', AV_PERSON_ID,'''',' AND T1.STATUS_CODE = 1' , LS_FILTER_CONDITION);
