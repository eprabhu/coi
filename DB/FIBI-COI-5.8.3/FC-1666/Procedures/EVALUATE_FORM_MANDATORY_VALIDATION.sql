        ...
        WHEN
            EXISTS( SELECT 
                ...
                FROM
                    fb_quest_answer A
                WHERE
                    A.QUESTION_ID = LI_QUESTION_ID
                        AND A.QUESTIONNAIRE_ANS_HEADER_ID = LI_ANS_HEADER_ID)
                OR EXISTS( SELECT 
                    1
                FROM
                    fb_quest_table_answer A
                WHERE
                    A.QUESTION_ID = LI_QUESTION_ID
                        AND A.QUESTIONNAIRE_ANS_HEADER_ID = LI_ANS_HEADER_ID)
        THEN
...
INTO LI_CNT;
   ...
   -- IF RULE_EVALUATION_FUNC(AV_MODULE_CODE, AV_MODULE_CODE, AV_SUBMODULE_CODE, AV_MODULE_ITEM_KEY,LI_RULE_ID, AV_SUB_MODULE_ITEM_KEY,AV_LOGGIN_PERSON_ID,AV_UPDATE_USER) = 0 THEN
         ITERATE QUEST_LOOP;
   -- END IF;
   END IF;
