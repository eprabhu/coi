...
-- Possbile Return values are NOT_REQUIRED, REQUIRED, TO_BE_DETERMINED
	...
	LS_CERTIFICATION_FLAG := FIBI_COI_DEV_PER_CERT_FLAG(AV_PROPOSAL_NUMBER,AV_PERSON_ID,AV_PROP_PERSON_NUMBER,AV_PROP_PERSON_ROLE_ID,AV_PERSON_PROJ_ROLE);
...
WHERE PROPOSAL_NUMBER = AV_PROPOSAL_NUMBER AND PROP_PERSON_NUMBER = AV_PROP_PERSON_NUMBER; 
    ...
    inner join questionnaire_answer t1 on t1.questionnaire_ah_id_fk = (select max(questionnaire_answer_header_id)
	from questionnaire_answer_header where module_Item_code = '3' and module_sub_item_code = LS_SUB_ITEM_CODE
	and substr(module_item_key,1,(instr(module_item_key,'|')-1)) = AV_PROPOSAL_NUMBER and module_sub_item_key =  AV_PERSON_ID)
