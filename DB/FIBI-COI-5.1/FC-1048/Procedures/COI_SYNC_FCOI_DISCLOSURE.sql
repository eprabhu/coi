    		...
    		IF LI_FCOI_TYPE_CODE != 2 THEN
    		    ...
    		    -- Deletes the projects from a FCOI disclosure based on certain conditions below
					...
					WITH AwardRanks AS (
							SELECT T1.AWARD_NUMBER,
								...
								ORDER BY CASE 
                                      ...
                                      WHEN (T1.DISCLOSURE_VALIDATION_FLAG = 'HIERARCHY' OR T1.DISCLOSURE_VALIDATION_FLAG IS NULL) 
						...
						SELECT GROUP_CONCAT(AWARD_NUMBER) INTO LS_AWARD_NUMBER 
						FROM AwardRanks
    				...
    				AND (
    					...
    					OR
    					(T1.MODULE_CODE = 1 AND FIND_IN_SET(T1.MODULE_ITEM_KEY, LS_AWARD_NUMBER) = 0)
    				);
    		    COMMIT;
            END IF;
    		START TRANSACTION;
