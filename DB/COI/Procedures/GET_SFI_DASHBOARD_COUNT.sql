...
END IF;
        ...
        SET LS_DYN_SQL = CONCAT('SELECT DISTINCT COUNT(*)  FROM (SELECT DISTINCT
                                        ...
                                        LEFT JOIN COUNTRY T7 ON T7.COUNTRY_CODE = T2.COUNTRY_CODE ', JOIN_CONDITION, ' WHERE T1.VERSION_STATUS != ''ARCHIVE'' AND T1.PERSON_ID = ''', AV_PERSON_ID , '''', TAB_QUERY, 
                                        ' GROUP BY T1.PERSON_ENTITY_ID) T ',LS_FILTER_CONDITION);
