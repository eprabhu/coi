...
END AS PROJECT_DISCLOSURE_REVIEW_STATUS
    ...
    INNER  JOIN coi_int_stage_award_person CISA ON CISA.PROJECT_NUMBER = T1.AWARD_NUMBER AND (CISA.NEW_DISCLOSURE_REQUIRED IS NULL OR CISA.NEW_DISCLOSURE_REQUIRED = 'N');
...
SET LS_DYN_SQL = CONCAT(
    ...
    'WITH CommentsCount AS (
	 ...
	 PersonCommentsCount AS (
        ...
        FROM DISCL_COMMENT
        where MODULE_CODE = 8
        GROUP BY DOCUMENT_OWNER_PERSON_ID
    )'
);
...
SELECT * FROM (SELECT 
    ...
    WHEN T1.PROJECT_SUBMISSION_STATUS = ''NOT_YET_DISCLOSED'' THEN ''Yet to disclose''
...
(SELECT FN_GET_OVAL_AWD_PRJT_DISCL_STATUS(T1.EXTERNAL_SYSTEM_REF_ID)) AS PROJECT_SUBMISSION_STATUS,
  ...
  COALESCE(PC.PERSON_COMMENT_COUNT, 0) AS PERSON_COMMENT_COUNT
	...
	LEFT JOIN PersonCommentsCount PC ON PC.DOCUMENT_OWNER_PERSON_ID = T1.KEY_PERSON_ID	
