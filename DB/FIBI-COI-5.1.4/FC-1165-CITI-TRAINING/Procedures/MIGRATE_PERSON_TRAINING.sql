...
DECLARE EXIT HANDLER FOR SQLEXCEPTION
	BEGIN	
		...
		SET @full_error = CONCAT("ERROR ", @errno, " (", @sqlstate, "): ", @msg);		
		 SELECT 'ERROR' AS STATUS, @full_error AS ERROR_MSG;				
	END;
...
-- PERSON ID IS NULL:
	...
	INSERT INTO CITI_TRAINING_FEED_EXCEPTION (ERROR_CODE,ERROR_MESSAGE,STACK_TRACE,ERROR_TYPE,ERROR_AT,FEED_INFO_ID,UPDATE_TIMESTAMP,UPDATED_BY)
	SELECT 'DB0002','Missing person id in CITI_TRAINING_FEED_DETAILS table',concat('Person ' , IFnull(LAST_NAME, '') , ', ' , IFnull(FIRST_NAME, '')  , ' is missing person id in CITI_TRAINING_FEED_DETAILS table'),'MIGRATION','PROCEDURE/MIGRATION_PERSON_TRAINING',AV_FEED_INFO_ID,UTC_TIMESTAMP(),'vineeth' FROM CITI_TRAINING_FEED_DETAILS
	WHERE CUSTOM_FIELD_2 IS NULL;

-- PERSON ID IS NOT PRESENT IN PERSON TABLE:

	INSERT INTO CITI_TRAINING_FEED_EXCEPTION (ERROR_CODE,ERROR_MESSAGE,STACK_TRACE,ERROR_TYPE,ERROR_AT,FEED_INFO_ID,UPDATE_TIMESTAMP,UPDATED_BY)                
    SELECT DISTINCT 'DB0002','Person does not exists in database',concat('Person id - ',T1.CUSTOM_FIELD_2),'MIGRATION','PROCEDURE/MIGRATION_PERSON_TRAINING',AV_FEED_INFO_ID,UTC_TIMESTAMP(),'vineeth'
	FROM CITI_TRAINING_FEED_DETAILS T1
    WHERE T1.CUSTOM_FIELD_2 IS NOT NULL
   ...
   END IF;
                    ...
                    IF (LI_CITI_DATA_STAGE_NUMBER = 2 or LI_CITI_DATA_STAGE_NUMBER = 3) THEN
                        ...
                        IF (LI_COUNT = 0) THEN
							...
							INSERT INTO CITI_TRAINING_FEED_EXCEPTION (ERROR_CODE,ERROR_MESSAGE,STACK_TRACE,ERROR_TYPE,ERROR_AT,FEED_INFO_ID,UPDATE_TIMESTAMP,UPDATED_BY)
							VALUES('DB0002','Person does not completed refresher but is missing basic couse',concat('Person id - ',LS_PERSON_ID),'MIGRATION','PROCEDURE/MIGRATION_PERSON_TRAINING',AV_FEED_INFO_ID,UTC_TIMESTAMP(),'vineeth');
						END IF;
                    ...
                    IF (LI_MODULE_CODE = 0) THEN
						...
						INSERT INTO CITI_TRAINING_FEED_EXCEPTION (ERROR_CODE,ERROR_MESSAGE,STACK_TRACE,ERROR_TYPE,ERROR_AT,FEED_INFO_ID,UPDATE_TIMESTAMP,UPDATED_BY)
						VALUES('DB0002','Module code is invalid',concat('person id - ',LS_PERSON_ID),'MIGRATION','PROCEDURE/MIGRATION_PERSON_TRAINING',AV_FEED_INFO_ID,UTC_TIMESTAMP(),'vineeth');
                       ...
                       ELSEIF (LI_MODULE_CODE = 104) THEN
                        ...
                        IF (LI_COUNT_HUMAN = 0) and (LI_COUNT_GCP = 0) and (LI_COUNT_RCR = 0) and (LI_COUNT_COI = 0) and (LI_COUNT_UFI = 0) and (LI_COUNT_IACUC = 0 and LI_COUNT_EXPORT = 0 ) THEN
                                ...
                                IF (LI_MODULE_CODE = 7) THEN
									...
									VALUES (LS_PERSON_ID, LI_TRAINING_NO + 1,  LI_TRAINING_CODE, LD_DATE_COMPLETED, UTC_TIMESTAMP(), LD_FOLLOWUP_DATE, LI_SCORE, UTC_TIMESTAMP(), 'vineeth','Y');
