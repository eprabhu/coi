    ...
    ELSEIF AV_MODULE_CODE = 3 AND AV_SUBMODULE_CODE in (0,4,1,3) THEN
        ...
        ELSEIF AV_MODULE_CODE = 23 AND AV_SUBMODULE_CODE = 0 THEN
                        SELECT CASE WHEN LOWER(LS_ARGUMENT_NAME) = LOWER('AV_PERSON_ID') THEN
								 ...
								 FROM DUAL;
                        select OPA_DISCLOSURE_ID, OPA_DISCLOSURE_NUMBER INTO  LI_OPA_DISCLOSURE_ID, LS_OPA_DISCLOSURE_NUMBER from OPA_DISCLOSURE
                             WHERE OPA_DISCLOSURE_ID = AV_MODULE_ITEM_KEY ;
        ELSEIF AV_MODULE_CODE = 8 AND AV_SUBMODULE_CODE = 801 THEN
                        SELECT CASE WHEN LOWER(LS_ARGUMENT_NAME) = LOWER('AV_PERSON_ENTITY_ID') THEN
			...
			ELSEIF AV_MODULE_CODE = 13 THEN
				...
				ELSEIF AV_MODULE_CODE = 8 AND AV_SUBMODULE_CODE = 0 THEN
                                SELECT CASE WHEN LOWER(LS_ARGUMENT_NAME) = LOWER('DISCLOSURE_ID') THEN
                                        ...
                                        WHEN LOWER(LS_ARGUMENT_NAME) = LOWER('AV_DISCLOSURE_ID') THEN
                                        COALESCE(AV_MODULE_ITEM_KEY,LS_DEFAULT_VALUE)
                                        END
                                INTO LS_ARGUMENT_VALUE
                                FROM DUAL;
                ELSEIF AV_MODULE_CODE = 24 AND AV_SUBMODULE_CODE = 0 THEN
                                ...
                                FROM DUAL;
                ELSEIF AV_MODULE_CODE = 23 AND AV_SUBMODULE_CODE = 0 THEN
                                 SET LS_ARGUMENT_VALUE = AV_MODULE_ITEM_KEY;
								 SELECT CASE WHEN LOWER(LS_ARGUMENT_NAME) = LOWER('AV_PERSON_ID') THEN
										COALESCE(AV_LOGGIN_PERSON_ID,LS_DEFAULT_VALUE)
										WHEN LOWER(LS_ARGUMENT_NAME) = LOWER('AV_OPA_DISCLOSURE_ID') THEN
                                        COALESCE(AV_MODULE_ITEM_KEY,LS_DEFAULT_VALUE)
								 ...
								 INTO LS_ARGUMENT_VALUE
								 FROM DUAL;
				ELSEIF AV_MODULE_CODE = 8 AND AV_SUBMODULE_CODE IN (14,1,2) THEN
								 SELECT CASE WHEN LOWER(LS_ARGUMENT_NAME) = LOWER('PERSON_ID') THEN
										COALESCE(AV_LOGGIN_PERSON_ID,LS_DEFAULT_VALUE)
										WHEN LOWER(LS_ARGUMENT_NAME) = LOWER('MODULE_ITEM_KEY') THEN
										COALESCE(AV_MODULE_ITEM_KEY,LS_DEFAULT_VALUE)
										END
...
CLOSE ARG_CURSOR;
    ...
    SELECT TRIM(TRAILING ',' FROM TRIM(LS_ARGUMENTS)) INTO LS_ARGUMENTS FROM DUAL;
                ...
                -- SET LL_QUERY_STRING = CONCAT('SELECT ',LS_DB_FUNCTION_NAME,'(',LS_ARGUMENTS,') INTO @LS_F_RESULT');
                SET LL_QUERY_STRING = CONCAT(
                    'SELECT CAST(', LS_DB_FUNCTION_NAME, '(', LS_ARGUMENTS, ') AS CHAR) INTO @LS_F_RESULT'
                );
