...
DECLARE LS_ADMIN_FULL_NAME VARCHAR(90);
...
EXIT_LABEL : BEGIN
   ...
   SET LS_DATE = utc_timestamp();
    ...
    IF AV_MODULE_CODE = 8 THEN
        SELECT FCOI_TYPE_CODE INTO LS_FCOI_TYPE_CODE FROM COI_DISCLOSURE WHERE DISCLOSURE_ID = AV_MODULE_ITEM_ID;
    END IF;
    ...
    ELSEIF AV_MODULE_CODE = 23 THEN
            IF AV_ACTION_TYPE IN('A','B') THEN
                SELECT CONCAT('OPA Disclosure of ', T2.FULL_NAME, ' submitted on ', DATE_FORMAT(T1.SUBMISSION_TIMESTAMP, '%m/%d/%Y')), T2.USER_NAME INTO LS_TITLE, LS_SUBMIT_USER FROM OPA_DISCLOSURE T1
                    INNER JOIN PERSON T2 ON T2.PERSON_ID = T1.PERSON_ID
                    WHERE T1.OPA_DISCLOSURE_ID = AV_MODULE_ITEM_ID;
                SET LS_MESSAGE_TYPE_CODE= '8027';
            ELSEIF AV_ACTION_TYPE = 'R' THEN
                SELECT FULL_NAME INTO LS_ADMIN_FULL_NAME
                FROM PERSON 
                WHERE PERSON_ID = AV_PERSON_ID;
                SELECT T2.USER_NAME, 
                    CONCAT('OPA disclosure returned by ', LS_ADMIN_FULL_NAME, ' on ', DATE_FORMAT(NOW(), '%m/%d/%Y'))
                INTO LS_SUBMIT_USER, LS_TITLE
                FROM OPA_DISCLOSURE T1
                INNER JOIN PERSON T2 ON T2.PERSON_ID = T1.PERSON_ID
                WHERE T1.OPA_DISCLOSURE_ID = AV_MODULE_ITEM_ID;
                SET LS_MESSAGE_TYPE_CODE = '8028';
            END IF;
            SET LS_MODULE_ITEM_ID = AV_MODULE_ITEM_ID;

    ELSEIF AV_MODULE_CODE = 8 THEN
        IF AV_ACTION_TYPE IN('A','B') THEN
            IF LS_FCOI_TYPE_CODE = 2 THEN
                SELECT CONCAT('Project disclosure for ', t3.DESCRIPTION, ' : ', COALESCE(t5.AWARD_NUMBER, t6.PROPOSAL_NUMBER), ' - ',
                COALESCE(t5.TITLE, t6.TITLE), ' of ', t2.FULL_NAME, ' submitted on ', DATE_FORMAT(UTC_TIMESTAMP(), '%m/%d/%Y')), t2.USER_NAME INTO LS_TITLE, LS_SUBMIT_USER
                ...
                INNER JOIN COI_PROJECT_AWARD_V t5 ON t5.COI_PROJECT_TYPE_CODE = t3.COI_PROJECT_TYPE_CODE AND t4.MODULE_ITEM_KEY = t5.AWARD_NUMBER
                INNER JOIN COI_PROJECT_PROPOSAL_V t6 ON t6.COI_PROJECT_TYPE_CODE = t3.COI_PROJECT_TYPE_CODE AND t6.PROPOSAL_NUMBER = t4.MODULE_ITEM_KEY
                WHERE t1.DISCLOSURE_ID = AV_MODULE_ITEM_ID;
                SET LS_MESSAGE_TYPE_CODE = '8037';
            ELSE
                SELECT CONCAT('FCOI disclosure of ', t2.FULL_NAME, ' submitted on ', DATE_FORMAT(UTC_TIMESTAMP(), '%m/%d/%Y')), t2.USER_NAME INTO LS_TITLE, LS_SUBMIT_USER
                FROM COI_DISCLOSURE t1
                INNER JOIN PERSON t2 ON t2.PERSON_ID = t1.PERSON_ID
                WHERE t1.DISCLOSURE_ID = AV_MODULE_ITEM_ID;
                SET LS_MESSAGE_TYPE_CODE = '8036';
            END IF;
        ELSEIF AV_ACTION_TYPE = 'R' THEN
            IF LS_FCOI_TYPE_CODE = 2 THEN
                SELECT CONCAT('Project disclosure for ', t3.DESCRIPTION, ' : ', COALESCE(t5.AWARD_NUMBER, t6.PROPOSAL_NUMBER), ' - ',
                COALESCE(t5.TITLE, t6.TITLE), ' returned by ', t2.FULL_NAME, ' on ', DATE_FORMAT(UTC_TIMESTAMP(), '%m/%d/%Y')), t2.USER_NAME INTO LS_TITLE, LS_SUBMIT_USER
                FROM COI_DISCLOSURE t1
                INNER JOIN PERSON t2 ON t2.PERSON_ID = AV_PERSON_ID
                INNER JOIN COI_PROJECT_TYPE t3 ON t3.COI_PROJECT_TYPE_CODE = t1.COI_PROJECT_TYPE_CODE
                INNER JOIN COI_DISCL_PROJECTS t4 ON t4.DISCLOSURE_ID = t1.DISCLOSURE_ID
                INNER JOIN COI_PROJECT_AWARD_V t5 ON t5.COI_PROJECT_TYPE_CODE = t3.COI_PROJECT_TYPE_CODE AND t4.MODULE_ITEM_KEY = t5.AWARD_NUMBER
                INNER JOIN COI_PROJECT_PROPOSAL_V t6 ON t6.COI_PROJECT_TYPE_CODE = t3.COI_PROJECT_TYPE_CODE AND t6.PROPOSAL_NUMBER = t4.MODULE_ITEM_KEY
                WHERE t1.DISCLOSURE_ID = AV_MODULE_ITEM_ID;
                SET LS_MESSAGE_TYPE_CODE = '153';
            ELSE
                SELECT CONCAT('FCOI disclosure returned by ', t2.FULL_NAME, ' on ', DATE_FORMAT(UTC_TIMESTAMP(), '%m/%d/%Y')), t2.USER_NAME INTO LS_TITLE, LS_SUBMIT_USER
                FROM COI_DISCLOSURE t1
                INNER JOIN PERSON t2 ON t2.PERSON_ID = AV_PERSON_ID
                WHERE t1.DISCLOSURE_ID = AV_MODULE_ITEM_ID;
                SET LS_MESSAGE_TYPE_CODE = '148';
            END IF;
        END IF;
        SET LS_MODULE_ITEM_ID = AV_MODULE_ITEM_ID;
    END IF;
...
IF AV_ACTION_TYPE = 'R' THEN
    ...
    IF AV_MODULE_CODE IN (1,3,5,13,14,16,23)  AND AV_SUBMODULE_CODE <> 2 THEN
    ...
    END IF;
		 ...
		 END IF;	   
		  ...
		  ELSE 
			   ...
			   END;
		   END IF;
ELSEIF AV_ACTION_TYPE = 'B' THEN -- BYPASS
...
SELECT T1.WORKFLOW_ID	
    ...
    */
	...
	IF LI_FLAG = 0 THEN
           ...
           END IF;
				-- ------------------------STARTS INSERTING INTO INBOX TABLE---------------------------------
				  SELECT COUNT(*) INTO LI_COUNT 
				  FROM WORKFLOW 
				  WHERE  WORKFLOW_ID = LL_WORKFLOW_ID AND MAP_TYPE = 'E';	
				...
				-- ------------------------ENDS INSERTING INTO INBOX TABLE-----------------------------------
