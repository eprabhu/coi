...
END IF;
	...
	-- -----------------------STARTS MODIFICATION ON 07-MAY-2020-------------------------
		...
		-- --------------------------change on 18/09/2020-------------
			...
			ELSEIF LS_MODULE_CODE = 8 THEN

		        SELECT FCOI_TYPE_CODE INTO LS_FCOI_TYPE_CODE
                        FROM COI_DISCLOSURE WHERE DISCLOSURE_ID = LS_MODULE_ITEM_ID;
		    END IF;
			-- --------------------------change on 18/09/2020-------------
		...
		ELSEIF LS_MODULE_CODE = 23 THEN
				SELECT CONCAT('OPA disclosure of ', T2.FULL_NAME, ' submitted on ', DATE_FORMAT(T1.SUBMISSION_TIMESTAMP, '%m/%d/%Y'))
				INTO LS_USER_MESSAGE
				FROM OPA_DISCLOSURE T1
				INNER JOIN PERSON T2 ON T2.PERSON_ID = T1.PERSON_ID
				WHERE T1.OPA_DISCLOSURE_ID = LS_MODULE_ITEM_ID;
		...
		ELSEIF LS_MODULE_CODE = 8 THEN
            IF LS_FCOI_TYPE_CODE = 2 THEN
                ...
                WHERE t1.DISCLOSURE_ID = LS_MODULE_ITEM_ID;
