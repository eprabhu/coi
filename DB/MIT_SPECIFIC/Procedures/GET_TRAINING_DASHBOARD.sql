...
ELSE
 SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.TRAINING_CODE = 54 AND ');
END IF;
...
END IF;
  ...
  SET LS_DYN_SQL =CONCAT('SELECT  *  FROM(SELECT 
    CASE 
        WHEN T1.IS_NON_EMPLOYEE = ''Y'' THEN R.FULL_NAME 
        ELSE P.FULL_NAME 
    END AS FULL_NAME,
    IFNULL(T2.DESCRIPTION, T3.CERTIFICATION_NAME) AS DESCRIPTION,
    T1.UPDATE_TIMESTAMP,
    T1.PERSON_ID,
    T1.IS_NON_EMPLOYEE,
    T1.PERSON_TRAINING_ID,
    T1.FOLLOWUP_DATE,
    T1.TRAINING_CODE,
    T1.DATE_ACKNOWLEDGED,
    T1.SCORE,
    T3.SOURCE_TYPE_CODE
    ' ,SELECTED_FIELD_LIST,'
FROM PERSON_TRAINING T1
LEFT JOIN ROLODEX R ON R.ROLODEX_ID = T1.PERSON_ID AND T1.IS_NON_EMPLOYEE = ''Y''
LEFT JOIN PERSON P ON P.PERSON_ID = T1.PERSON_ID AND T1.IS_NON_EMPLOYEE = ''N''
LEFT JOIN TRAINING T2 ON T1.TRAINING_CODE = T2.TRAINING_CODE
LEFT JOIN PERSON_TRAINING_EXT T3 ON T1.PERSON_TRAINING_ID = T3.PERSON_TRAINING_ID
