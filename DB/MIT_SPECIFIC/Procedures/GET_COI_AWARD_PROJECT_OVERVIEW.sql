...
FROM (SELECT 
   ...
   CASE
    ...
    END AS PERSON_SUBMISSION_STATUS ,
 	CASE   
    WHEN T1.DISCLOSURE_REVIEW_STATUS = ''DISCLOSURE_COMPLETED'' THEN ''Approved''
	...
	WHEN T1.DISCLOSURE_REVIEW_STATUS = ''DISCLOSURE_PENDING'' THEN ''In Progress''    
...
(SELECT DISTINCT 
    ...
    T1.PROJECT_STATUS,
	COALESCE(T2.MAX_DISCLOSURE_ID, RELATED.MAX_DISCLOSURE_ID) AS DISCLOSURE_ID,
  ...
  CASE
                ...
                CASE
                    ...
                    WHEN EXISTS (
                        ...
                        WHERE SDR.SPONSOR_CODE IN (T1.SPONSOR_CODE, T1.PRIME_SPONSOR_CODE)
                          ...
                          AND T2.KEY_PERSON_ROLE_CODE IN (''1'',''3'')
	...
	FROM COI_INT_STAGE_AWARD T1		
	 ...
	 LEFT JOIN PersonCommentsCount PC 
	 	ON PC.MODULE_ITEM_KEY = T2.MAX_DISCLOSURE_ID
	 	AND PC.DOCUMENT_OWNER_PERSON_ID = T2.KEY_PERSON_ID
	LEFT JOIN (
			SELECT
				SUBSTRING_INDEX(CISAP.PROJECT_NUMBER, ''-'', 1) AS PROJECT_PREFIX,
				CISAP.KEY_PERSON_ID,
				MAX(CISAP.MAX_DISCLOSURE_ID) AS MAX_DISCLOSURE_ID
			FROM COI_INT_STAGE_AWARD_PERSON CISAP
			WHERE CISAP.MAX_DISCLOSURE_ID IS NOT NULL
			GROUP BY
				SUBSTRING_INDEX(CISAP.PROJECT_NUMBER, ''-'', 1),
				CISAP.KEY_PERSON_ID
		) RELATED ON RELATED.PROJECT_PREFIX = SUBSTRING_INDEX(T1.PROJECT_NUMBER, ''-'', 1)
				   AND RELATED.KEY_PERSON_ID = T2.KEY_PERSON_ID
