...
export function getInvalidDateFormatMessage(date?: string | null): string | null {    
    if (!date || isValidDateFormat({ _i: date })) {
        return null;
    }
    return `Entered date format is invalid. Please use ${DEFAULT_DATE_FORMAT} format.`;
}
