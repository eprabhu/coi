import { Component, Input, OnDestroy, OnInit } from '@angular/core';
import { fadeDown } from '../../../common/utilities/animations';
import { GoogleChartService } from '../../../research-summary-widgets/google-chart.service';
import { ElasticMonitoringService } from '../elastic-monitoring.service';

@Component({
  selector: 'app-elastic-message-received',
  templateUrl: './elastic-message-received.component.html',
  styleUrls: ['./elastic-message-received.component.css'],
  animations: [fadeDown]
})


export class ElasticMessageReceivedComponent extends GoogleChartService implements OnInit {

  private chart;
  private axisValues;
  private data;
  @Input() isShowLoader = false;
  messagesReceived = [];

  constructor(private _elasticMonitoringService: ElasticMonitoringService) {
    super();
  }

  ngOnInit() {
    this.getListOfMessagesReceived();
  }

  getListOfMessagesReceived() {
    this._elasticMonitoringService.$elasticMonitoringDetails.subscribe((data: any) => {
      if (data) {
        this.messagesReceived = data.ElasticQueue.NumberOfMessagesReceived || [];
        if (this.messagesReceived.length > 0) {
          this.googleChartFunction();
        }
      }
    });
  }
  drawGraph() {
    this.data = this.createcolumnChartTable();
    this.data.addColumn('datetime', 'Date');
    this.data.addColumn('number', 'Count');
    this.messagesReceived.sort((a, b) => a.timestamp - b.timestamp);
    this.messagesReceived.forEach((ele: any) => {
      this.data.addRows([[new Date(ele.timestamp), ele.sum]]);
    });
    this.axisValues = {
      legend: { position: 'top', alignment: 'start' },
      colors: ['green'],
      hAxis: {
        title: 'Date'
      },
      vAxis: {
        title: 'Count'
      }
    };
    this.chart = this.createLineChart(document.getElementById('elastic_monitoring_line_chart_message_received'));
    this.chart.draw(this.data, this.axisValues);
  }


  onResize(event) {
    if (this.chart) {
      this.chart.draw(this.data, this.axisValues);
    }
  }

}
